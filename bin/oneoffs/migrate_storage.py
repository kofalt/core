    ...
    for container, prefix in containers_prefixes:
        ...
        for document in cursor:
            for f in get_files_by_prefix(document, prefix):
                f_dict = {"container_id": document.get("_id"), "container": container, "fileinfo": f, "prefix": prefix}
...
def get_gears_files():
    ...
    for document in cursor:
        if document.get("exchange", {}).get("git-commit", "") == "local":
            f_dict = {"gear_id": document["_id"], "gear_name": document["gear"]["name"], "exchange": document["exchange"]}
            _files.append(f_dict)
        ...
        def f_retry(*args, **kwargs):
            ...
            while mtries > 1:
                try:
                    ...
                    f_to_migrate = args[0]
                    msg = "Couldn't upload %s/%s/%s to target storage, retrying in %ds (%s)..." % (f_to_migrate["container"], f_to_migrate["container_id"], f_to_migrate["fileinfo"]["name"], mdelay, tries - mtries + 1)
                    log.warning(msg)
                    time.sleep(mdelay)
                    mtries -= 1
                    mdelay *= backoff
...
def migrate_file(f):
    ...
    if file_id:
        ...
        update_set = {f["prefix"] + ".$.modified": datetime.datetime.utcnow(), f["prefix"] + ".$._id": file_id}

        # Update the file with the newly generated UUID
        updated_doc = db[f["container"]].find_one_and_update({"_id": f["container_id"], f["prefix"] + ".name": f["fileinfo"]["name"], f["prefix"] + ".hash": f["fileinfo"]["hash"]}, {"$set": update_set})

        if not updated_doc:
            log.info("Probably the following file has been updated during the migration " "and its hash is changed, cleaning up from the new filesystem")
            target_fs.remove_file(file_id, f_new_path)


def migrate_analysis_file(f, migrated_files):
    match = [cf for cf in migrated_files if cf["fileinfo"]["hash"] == f["fileinfo"]["hash"] and cf["fileinfo"].get("_id")]
    ...
    if len(match) > 0 and not f["fileinfo"].get("_id"):
        ...
        update_set = {f["prefix"] + ".$.modified": match[0]["fileinfo"]["modified"], f["prefix"] + ".$._id": match[0]["fileinfo"]["_id"]}

        # Update the file with the newly generated UUID
        db[f["container"]].find_one_and_update({"_id": f["container_id"], f["prefix"] + ".name": f["fileinfo"]["name"], f["prefix"] + ".hash": f["fileinfo"]["hash"]}, {"$set": update_set})
    else:
...
def migrate_containers():
    ...
    container_files = get_containers_files([("projects", "files"), ("acquisitions", "files"), ("sessions", "files"), ("subjects", "files"), ("collections", "files"), ("analyses", "files")])

    for i, f in enumerate(container_files):
        log.debug("  [%s/%s] %s/%s/%s", i + 1, len(container_files), f["container"], f["container_id"], f["fileinfo"]["name"])
        migrate_file(f)
        show_progress(i + 1, len(container_files))

    log.info("Migrate analysis files...")
    # Refresh the list of container files
    migrated_files = get_containers_files([("projects", "files"), ("acquisitions", "files"), ("sessions", "files"), ("subjects", "files"), ("collections", "files"), ("analyses", "files")])
...
def migrate_gear_files(f):
    ...
    if file_id:
        ...
        update_set = {"modified": datetime.datetime.utcnow(), "exchange.rootfs-id": file_id}

        # Update the gear with the newly generated UUID
        db["gears"].find_one_and_update({"_id": f["gear_id"]}, {"$set": update_set})
