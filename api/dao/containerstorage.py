...
class GroupStorage(ContainerStorage):
    ...
    def create_el(self, payload):
        ...
        return self.dbc.update_one({"_id": payload["_id"]}, {"$set": payload, "$setOnInsert": {"permissions": permissions, "created": created}}, upsert=True)
...
class AnalysisStorage(ContainerStorage):
    ...
    def create_el(self, analysis, parent_type, parent_id, origin, uid=None):
        ...
        * Flatten input filerefs using `FileReference.get_file()`
        ...
        defaults = {"_id": bson.ObjectId(), "parent": {"type": parent_type, "id": bson.ObjectId(parent_id)}, "created": datetime.datetime.utcnow(), "modified": datetime.datetime.utcnow(), "user": origin.get("id")}
        ...
        if job is not None:
            ...
            try:
                ...
                self.update_el(analysis["_id"], {"job": job.id_, "gear_info": gear_info}, None)
            except:
...
class QueryStorage(ContainerStorage):
    ...
    def get_all_el(self, query, user, projection, fill_defaults=False, pagination=None, **kwargs):
        ...
        if user is not None:
            query = query or {}
            query.update({"parent.id": {"$in": self._get_user_parents(user["_id"], user.get("root", False))}})
        return super(QueryStorage, self).get_all_el(query, None, projection, fill_defaults=fill_defaults, pagination=pagination, **kwargs)
