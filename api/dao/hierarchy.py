    ...
    for f in container_before.get("files", []):
        ...
        if f["name"] == fileinfo["name"]:
            ...
            # The file object is the same as an existing file and the caller has chosen to ignore in this situation
                ...
                saved_state = "ignored"
                logger.info("File id=%s, name=<%s> was ignored because its hash <%s> matches the existing hash <%s> on %s=%s", fileinfo["_id"], fileinfo["name"], fileinfo["hash"], f.get("hash"), cont_name, _id)
...
def add_file(cont_name, _id, fileinfo):
    ...
    return config.db[cont_name].find_one_and_update({"_id": _id}, {"$push": {"files": fileinfo}}, return_document=pymongo.collection.ReturnDocument.AFTER)
...
def update_file(cont_name, _id, fileinfo):
    ...
    return config.db[cont_name].find_one_and_update({"_id": _id, "files.name": fileinfo["name"]}, {"$set": update_set}, return_document=pymongo.collection.ReturnDocument.AFTER)
...
def replace_file(cont_name, _id, existing_fileinfo, fileinfo, access_logger):
    ...
    return config.db[cont_name].find_one_and_update({"_id": _id, "files.name": fileinfo["name"]}, {"$set": {"files.$": fileinfo}}, return_document=pymongo.collection.ReturnDocument.AFTER)


def remove_file(cont_name, _id, filename):
    return config.db[cont_name].find_one_and_update({"_id": _id, "files.name": filename}, {"$pull": {"files": {"name": filename}}}, return_document=pymongo.collection.ReturnDocument.AFTER)
...
def _find_or_create_destination_project(group_id, project_label, timestamp, user, unsorted_projects):
    ...
    if not project:
        ...
        project = {"group": group["_id"], "label": project_label, "permissions": group["permissions"], "public": False, "created": timestamp, "modified": timestamp}

        if unsorted_projects:
            project[
                "description"
            ] = "This project was automatically created because unsortable data was detected. \
...
def _create_query(cont, cont_type, parent_type, parent_id, upload_type):
    if upload_type in ("label", "uid"):
        ...
        return {parent_type: bson.ObjectId(parent_id), match_key: cont[match_key], "deleted": {"$exists": False}}
    else:
...
def _upsert_container(cont, cont_type, parent, parent_type, upload_type, timestamp):
    ...
    if config.db[cont_name].find_one(query) is not None:
        ...
        insert_vals = {parent_type: parent["_id"], "permissions": parent["permissions"], "public": parent.get("public", False), "created": timestamp}
...
def _get_targets(project_obj, session, acquisition, type_, timestamp):
    ...
    target_containers.append((TargetContainer(subject_obj, "subject"), subject_files))
    ...
    target_containers.append((TargetContainer(session_obj, "session"), session_files))
    ...
    target_containers.append((TargetContainer(acquisition_obj, "acquisition"), acquisition_files))
    return target_containers
...
def find_existing_hierarchy(metadata, type_="uid", user=None):
    ...
    target_containers.append((TargetContainer(project_obj, "project"), project_files))
    return target_containers
