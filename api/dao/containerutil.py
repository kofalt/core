...
CONT_TYPES = ["acquisition", "session", "subject", "project", "group", "analysis", "collection"]
SINGULAR_TO_PLURAL = {"acquisition": "acquisitions", "analysis": "analyses", "collection": "collections", "device": "devices", "group": "groups", "job": "jobs", "project": "projects", "session": "sessions", "subject": "subjects", "user": "users", "query": "queries"}
...
CONTAINER_HIERARCHY = ["groups", "projects", "subjects", "sessions", "acquisitions"]
...
def get_project_groups(uid):
    ...
    """
    pipeline = [{"$match": {"permissions._id": uid}}, {"$group": {"_id": "$group"}}]
...
def get_project_stats(project_list):
    ...
    match_q = {"project": {"$in": project_ids}, "deleted": {"$exists": False}}

    # Get session / compliance counts
    pipeline = [{"$match": match_q}, {"$project": {"_id": 1, "project": 1, "non_compliant": {"$cond": [{"$eq": ["$satisfies_template", False]}, 1, 0]}}}, {"$group": {"_id": "$project", "noncompliant_count": {"$sum": "$non_compliant"}, "total": {"$sum": 1}}}]
    session_results_map = {row["_id"]: row for row in config.db.sessions.aggregate(pipeline)}

    # Get subject count
    pipeline = [{"$match": match_q}, {"$group": {"_id": "$project", "total": {"$sum": 1}}}]
...
def get_collection_stats(cont):
    ...
    session_ids = set()
    subject_ids = set()
    for row in config.db.acquisitions.find({"collections": cont["_id"], "deleted": {"$exists": False}}, {"session": 1, "parents.subject": 1}):
    ...
    pipeline = [{"$match": match_q}, {"$project": {"_id": 1, "non_compliant": {"$cond": [{"$eq": ["$satisfies_template", False]}, 1, 0]}}}, {"$group": {"_id": 1, "noncompliant_count": {"$sum": "$non_compliant"}, "total": {"$sum": 1}}}]
...
def get_referring_analyses(cont_name, cont_id, filename=None):
    ...
    """
    query = {"destination.type": "analysis", "inputs.type": singularize(cont_name), "inputs.id": str(cont_id)}
    if filename:
...
class ContainerReference(object):
    ...
    def from_filereference(cls, fr):
        return cls(type=fr.type, id=fr.id)
    ...
    def get(self):
        ...
        if "parent" in result:
            ...
            if parent is None:
                raise APINotFoundException("Cannot find parent {} {} of {} {}".format(result["parent"]["type"], result["parent"]["id"], self.type, self.id))
...
class FileReference(ContainerReference):
    ...
    @classmethod
    def from_dictionary(cls, d):
        return cls(type=d["type"], id=d["id"], name=d["name"])
