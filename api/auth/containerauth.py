        ...
        def f(method, _id=None, payload=None, unset_payload=None, recursive=False, r_payload=None, replace_metadata=False):
            ...
            if method == "GET" and container.get("public", False):
                ...
                if target_parent_container.get("cont_name") in ["project", "session", "subject"]:
                    ...
                    required_perm = "admin"
                has_access = _get_access(handler.uid, container, scope=handler.scope) >= INTEGER_PERMISSIONS[required_perm] and _get_access(handler.uid, target_parent_container, scope=handler.scope) >= INTEGER_PERMISSIONS[required_perm]
...
def list_permission_checker(handler):
    def g(exec_op):
        def f(method, query=None, user=None, public=False, projection=None, pagination=None):
            if handler.scope:
                query["$or"] = [{"parents.{}".format(handler.scope["level"]): handler.scope["id"]}, {"_id": handler.scope["id"]}, {"public": True}]
