...
class CollectionsHandler(ContainerHandler):
    ...
    container_handler_configurations["collections"] = {"permchecker": containerauth.collection_permissions, "storage": containerstorage.CollectionStorage(), "storage_schema_file": "collection.json", "payload_schema_file": "collection.json"}
    ...
    def curators(self):
        ...
        curators = config.db.users.find({"_id": {"$in": curator_ids}}, ["firstname", "lastname"])
    ...
    def get_sessions(self, cid):
        ...
        agg_res = config.db.acquisitions.aggregate([{"$match": {"collections": bson.ObjectId(cid)}}, {"$group": {"_id": "$session"}}])
