...
ANALYSIS = {"analyzer": {"my_analyzer": {"tokenizer": "my_tokenizer", "filter": ["lowercase"]}}, "tokenizer": {"my_tokenizer": {"type": "ngram", "min_gram": 2, "max_gram": 100, "token_chars": ["letter", "digit", "symbol", "punctuation"]}}}

DYNAMIC_TEMPLATES = [{"string_fields": {"match": "*", "match_mapping_type": "string", "mapping": {"type": "text", "analyzer": "my_analyzer", "search_analyzer": "standard", "index": True, "fields": {"raw": {"type": "keyword", "index": True, "ignore_above": 256}}}}}]
...
FACET_QUERY = {
    "size": 0,
    "aggs": {
        "session_count": {"cardinality": {"field": "session._id"}},
        "acquisition_count": {"cardinality": {"field": "acquisition._id"}},
        "analysis_count": {"cardinality": {"field": "analysis._id"}},
        "file_count": {"cardinality": {"field": "file._id"}},
        "by_session": {
            "filter": {"term": {"container_type": "session"}},
            "aggs": {
                "subject.sex": {"terms": {"field": "subject.sex.raw", "size": 15, "missing": "null"}},
                "session.tags": {"terms": {"field": "subject.tags.raw", "size": 15, "missing": "null"}},
                "subject.code": {"terms": {"field": "subject.code.raw", "size": 15, "missing": "null"}},
                "session.timestamp": {"stats": {"field": "session.timestamp"}},
            },
        },
        "session_age": {
            "filter": {"bool": {"must": [{"range": {"session.age": {"gte": -31556952, "lte": 3155695200}}}, {"term": {"container_type": "session"}}]}},
            "aggs": {"session.age": {"histogram": {"field": "session.age", "interval": 31556952, "extended_bounds": {"min": -31556952, "max": 3155695200}}}},
        },
        "by_file": {
            "filter": {"term": {"container_type": "file"}},
            "aggs": {
                "file.classification_list": {"terms": {"field": "file.classification_list.raw", "size": 15, "missing": "null"}},
                "file.modality": {"terms": {"field": "file.modality.raw", "size": 15, "missing": "null"}},
                "file.type": {"terms": {"field": "file.type.raw", "size": 15, "missing": "null"}},
            },
        },
    },
}
...
SOURCE_COMMON = ["group._id", "group.label", "permissions"]

SOURCE_COLLECTION = ["permissions", "collection._id", "collection.label", "collection.curator", "collection.created"]

SOURCE_PROJECT = SOURCE_COMMON + ["project._id", "project.label"]

SOURCE_SESSION = SOURCE_PROJECT + ["session._id", "session.created", "session.label", "session.timestamp", "subject.code"]

SOURCE_ACQUISITION = SOURCE_SESSION + ["acquisition._id", "acquisition.created", "acquisition.label", "acquisition.timestamp"]

SOURCE_ANALYSIS = SOURCE_SESSION + ["analysis._id", "analysis.created", "analysis.label", "analysis.user", "analysis.parent"]  # TODO: coalesce analysis and file parent keys (analysis.parent.id vs parent._id for file)

SOURCE_FILE = SOURCE_ANALYSIS + ["file.created", "file.classification", "file.name", "file.size", "file.type", "file.mimetype", "parent"]

SOURCE = {"collection": SOURCE_COLLECTION, "project": SOURCE_PROJECT, "session": SOURCE_SESSION, "acquisition": SOURCE_ACQUISITION, "analysis": SOURCE_ANALYSIS, "file": SOURCE_FILE}
...
class DataExplorerHandler(base.RequestHandler):
                ...
                for k, v in f["terms"].iteritems():
                    if "null" in v:
                        if isinstance(v, list):
                            ...
                            v = None
                        null_filter = {"bool": {"should": [{"bool": {"must": [{"bool": {"must_not": [{"exists": {"field": k}}]}}]}}]}}
                        if len(k.split(".")) > 1:
    ...
    def get_facets(self):
        ...
        aggs = config.es.search(index="data_explorer", doc_type="flywheel", body=facets_q)["aggregations"]
    ...
    def search_size(self, return_type, filters=None):
        body = {"size": 0, "aggs": {"count": {"cardinality": {"field": return_type + "._id", "precision_threshold": 100000}}}}

        if filters:
            body["query"] = {"bool": {"filter": filters}}
        size = config.es.search(index="data_explorer", doc_type="flywheel", body=body)["aggregations"]["count"]["value"]
        size = int(size * 1.02)
        return size
    ...
    def suggest(self):
        ...
        return {"from": parse_result.pos, "suggestions": [{"display": s, "value": search.escape_id(s)} for s in suggestions]}
    ...
    def get_nodes(self):
        ...
        nodes = []
        results = config.es.search(index="data_explorer", doc_type="flywheel", body=body)["aggregations"]["by_container"]["buckets"]
    ...
    def _suggest_fields(self, field_query, count=15):
        es_query = {"size": count, "query": {"match": {"name": field_query}}}
        try:
            es_results = config.es.search(index="data_explorer_fields", doc_type="flywheel_field", body=es_query)
    ...
    def _suggest_values(self, field_name, include_stats=True, search_string=None, count=15):
        ...
        body = {"size": 0, "query": {"bool": {"must": {"match": {field_name: search_string}}, "filter": filters}}}
        if not filters:
        ...
        if field_type in ["string", "boolean"]:
            body["aggs"] = {"results": {"terms": {"field": field_name + ".raw", "size": count, "missing": "null"}}}
        ...
        aggs = config.es.search(index="data_explorer", doc_type="flywheel", body=body)["aggregations"]["results"]
        return aggs
    ...
    def save_training_set(self):
        ...
        if payload.get("search_query"):
            ...
            file_results = config.es.mget(index="data_explorer", body={"docs": docs})["docs"]
        else:
        ...
        formatted_search_results = {"dataset": {"description": payload.get("description"), "labels": labels}, "files": file_results}
        ...
        placer = TargetedMultiPlacer(output["type"], output_container, output["id"], metadata, timestamp, self.origin, {"uid": self.uid}, self.log_user_access)
    ...
    def _construct_query(self, return_type, search_string, filters, size=100):
        ...
        query = {"size": 0, "query": {"bool": {"must": {"match": {"_all": search_string}}, "filter": {"bool": {"must": filters}}}}}

        if return_type:  # only searches have a return type, not facet queries
            query["aggs"] = {"by_container": {"terms": {"field": return_type + "._id", "size": size}, "aggs": {"by_top_hit": {"top_hits": {"_source": SOURCE[return_type], "size": 1}}}}}
    ...
    def _construct_exact_query(self, return_type, search_string, filters, size=100):
        query = {"size": size, "_source": SOURCE[return_type], "query": {"bool": {"must": {"match": {"_all": ""}}, "filter": {"bool": {"must": [{"term": {"container_type": return_type}}]}}}}}

        if return_type == "file":
            query["script_fields"] = {"info_exists": INFO_EXISTS_SCRIPT}
    ...
    def _run_query(self, es_query, result_type):
        try:
            results = config.es.search(index="data_explorer", doc_type="flywheel", body=es_query)
        except RequestError:
    ...
    def _handle_properties(cls, properties, current_field_name):
        ...
        ignore_fields = ["_all", "dynamic_templates", "analysis_reference", "file_reference", "parent", "container_type", "origin", "permissions", "_id", "project_has_template", "hash"]
        ...
        for field_name, field in properties.iteritems():
            ...
            if field_name in ignore_fields:
                ...
                if field_type == "string":
                    ...
                    body = {"size": 0, "aggs": {"results": {"terms": {"field": field_name + ".raw", "size": 15}}}}
                ...
                doc = {"name": field_name, "type": field_type, "facet": facet_status}
    ...
    def index_field_names(self):
        ...
        if not config.es.indices.exists("data_explorer_fields"):
            request = {"settings": {"number_of_shards": 1, "number_of_replicas": 0, "analysis": ANALYSIS}, "mappings": {"_default_": {"_all": {"enabled": True}, "dynamic_templates": DYNAMIC_TEMPLATES}, "flywheel": {}}}
