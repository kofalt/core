...
class ContainerHandler(base.RequestHandler):
    ...
    use_object_id = {"groups": False, "projects": True, "subjects": True, "sessions": True, "acquisitions": True}
    ...
    container_handler_configurations = {
        ...
        "subjects": {"storage": containerstorage.SubjectStorage(), "permchecker": containerauth.default_container, "parent_storage": containerstorage.ProjectStorage(), "storage_schema_file": "subject.json", "payload_schema_file": "subject.json", "children_cont": "sessions"},
        "sessions": {"storage": containerstorage.SessionStorage(), "permchecker": containerauth.default_container, "parent_storage": containerstorage.ProjectStorage(), "storage_schema_file": "session.json", "payload_schema_file": "session.json", "children_cont": "acquisitions"},
        "acquisitions": {"storage": containerstorage.AcquisitionStorage(), "permchecker": containerauth.default_container, "parent_storage": containerstorage.SessionStorage(), "storage_schema_file": "acquisition.json", "payload_schema_file": "acquisition.json"},
    }
    ...
    def get_jobs(self, cid):
        ...
        cont_refs = [containerutil.ContainerReference(cont_type, str(cont_id)) for cont_type, cont_id in [("session", cont["_id"])] + [("analysis", an["_id"]) for an in analyses] + [("acquisition", aq["_id"]) for aq in acquisitions]]
    ...
    def _add_results_counts(self, results, cont_name):
        ...
        counts = dbc.aggregate([{"$match": {el_cont_name: {"$in": [res["_id"] for res in results]}, "deleted": {"$exists": False}}}, {"$group": {"_id": "$" + el_cont_name, "count": {"$sum": 1}}}])
    ...
    def post(self, cont_name):
        ...
        if cont_name == "subjects":
            ...
            if self.storage.get_all_el({"project": bson.ObjectId(payload["project"]), "code": subject_code}, None, {"_id": 1}):
    ...
    def put(self, cont_name, **kwargs):
        ...
        if cont_name == "subjects":
            ...
            if subject_code and self.storage.get_all_el({"project": project_id, "code": subject_code, "_id": {"$ne": container["_id"]}}, None, {"_id": 1}):  # Make sure that if neither code nor project changed, we allow it
        ...
        if cont_name == "sessions":
            ...
            # Check for subject code collision 1st when changing project and/or subject code
            if (target_project and project_id != current_project["_id"]) or (target_subject_code and subject_code != current_subject.get("code")):
        ...
        # and checking permissions using respectively the two decorators, mongo_validator and permchecker
        result = mongo_validator(permchecker(self.storage.exec_op))("PUT", _id=_id, payload=payload, recursive=rec, r_payload=r_payload, replace_metadata=replace_metadata)
