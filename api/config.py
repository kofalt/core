...
logging.basicConfig(format="%(asctime)s %(name)16.16s %(filename)24.24s %(lineno)5d:%(levelname)4.4s %(message)s", datefmt="%Y-%m-%d %H:%M:%S", level=logging.DEBUG)
...
DEFAULT_CONFIG = {
    "core": {"debug": False, "log_level": "info", "access_log_enabled": False, "drone_secret": None},
    "site": {"id": "local", "name": "Local", "api_url": "https://localhost/api", "redirect_url": "https://localhost", "central_url": "https://sdmc.scitran.io/api", "registered": False, "ssl_cert": None, "inactivity_timeout": None, "upload_maximum_bytes": "10737418240"},
    "queue": {"max_retries": 3, "retry_on_fail": False},
    "auth": {
        "google": {
    ...
    "master_subject_code": {"size": "6", "prefix": "fw", "chars": "BCDFGHJKLMNPQRSTVWXYZ123456789", "verify_config": None},
}
...
def initialize_db():
    ...
    db.subjects.create_index([("project", 1), ("code", 1), ("deleted", 1)], partialFilterExpression={"code": {"$exists": True}}, unique=True, **kwargs)
    db.subjects.create_index([("project", 1), ("master_code", 1), ("deleted", 1)], partialFilterExpression={"master_code": {"$exists": True}}, unique=True, **kwargs)
    ...
    db.master_subject_codes.create_index([("first_name", 1), ("last_name", 1), ("date_of_birth", 1), ("patient_id", 1)], unique=True, **kwargs)
    db.master_subject_codes.create_index("patient_id", partialFilterExpression={"patient_id": {"$exists": True}}, unique=True, **kwargs)
    ...
    try_update_one(db, "groups", {"_id": "unknown"}, {"$setOnInsert": {"created": now, "modified": now, "label": "Unknown", "permissions": []}}, upsert=True)
...
def get_public_config():
    ...
    return {"created": __config.get("created"), "modified": __config.get("modified"), "site": __config.get("site"), "auth": auth, "signed_url": cfg["features"]["signed_url"], "features": cfg["features"]}  # Legacy note: clients expect top-level signed_url key
