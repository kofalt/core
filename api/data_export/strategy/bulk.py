    ...
    def identify_targets(self, spec, uid, summary):
        ...
        for fref in spec.get("files", []):
            ...
            try:
                ...
                file_obj = config.db[coll_name].find_one(query, {"files": {"$elemMatch": {"name": filename}}})["files"][0]
            ...
            if src_path:  # silently skip missing files
                ...
                yield models.DownloadTarget("file", dst_path, cont_name, cont_id, file_obj["modified"], file_obj["size"], file_obj.get("type"), file_id=file_obj.get("_id"), filename=file_obj["name"], src_path=src_path)
            else:
