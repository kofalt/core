...
class FullDownloadStrategy(HierarchyDownloadStrategy):
    ...
    def __init__(self, log, params):
        ...
        self._resolver = ContainerPathResolver(path_prefix=self.archive_prefix, include_group=False, prefix_containers=True)
    ...
    def visit_container(self, parents, container_type, container, summary):
        ...
        metadata_target = self._create_metadata_target(container_path, container_type, container_id, container)
        ...
        if src_path:  # silently skip missing files
            ...
            if summary:
                ...
                if self.include_metadata:
                    ...
                    metadata_target = self._create_metadata_target(file_path, parent_type, parent_id, file_entry, file_id=file_id, filename=filename, file_group=file_group)

                dst_path = "/".join(file_path)

            result = [models.DownloadTarget("file", dst_path, parent_type, parent_id, file_entry["modified"], file_entry["size"], file_entry.get("type"), file_id=file_id, filename=filename, file_group=file_group, src_path=src_path)]
    ...
    def _create_metadata_target(self, path, parent_type, parent_id, container, file_id=None, filename=None, file_group=None):
        ...
        return models.DownloadTarget("metadata_sidecar", dst_path, parent_type, parent_id, container["modified"], 0, "metadata", file_id=file_id, filename=filename, file_group=file_group)
