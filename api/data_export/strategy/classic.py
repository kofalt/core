    ...
    def visit_file(self, parents, parent_type, file_group, file_entry, summary):
        ...
        if src_path:  # silently skip missing files
            if summary:
                ...
                dst_path = "{}/{}/{}".format(parent.get("label", "unknown_analysis"), file_group, file_entry["name"])
            else:
            ...
            return [models.DownloadTarget("file", dst_path, parent_type, parent_id, file_entry["modified"], file_entry["size"], file_entry.get("type"), file_id=file_entry.get("_id"), filename=file_entry["name"], file_group=file_group, src_path=src_path)]
        else:
