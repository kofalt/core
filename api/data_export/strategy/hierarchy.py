...
CONTAINER_LEVELS = {"project": 1, "subject": 2, "session": 3, "acquisition": 4, "analysis": 5}
...
class HierarchyDownloadStrategy(AbstractDownloadStrategy):
        ...
        if summary:
            ...
            for node in spec:
                ...
                if level not in self.projection_map:
                    raise errors.InputValidationException("{} is not a recognized level".format(level))
        else:
    ...
    def identify_targets(self, spec, uid, summary):
        ...
        while self.visit_nodes:
            ...
            for file_group, file_entry in files:
                for target in self.visit_file(parents, container_type, file_group, file_entry, summary):
                    yield target
    ...
    def _create_visit_tree(self, nodes):
        ...
        result = collections.OrderedDict([("project", collections.deque()), ("subject", collections.deque()), ("session", collections.deque()), ("acquisition", collections.deque()), ("analysis", collections.deque())])
