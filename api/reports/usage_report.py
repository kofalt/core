...
class UsageReport(Report):
    ...
    def _create_default(self, month=None, year=None, project=None, ignore_minmax=False):
        ...
        """
        obj = {"gear_execution_count": 0, "file_mbs": 0, "session_count": 0}
        if month:
            obj["month"] = month
        if year:
            obj["year"] = year
    ...
    def _build_month_report(self, base_query):
        ...
        pipeline = [{"$match": job_q}, {"$project": {"month": {"$month": "$created"}, "year": {"$year": "$created"}}}, {"$group": {"_id": {"month": "$month", "year": "$year"}, "jobs_completed": {"$sum": 1}}}]
        ...
        pipeline = [{"$match": base_query}, {"$project": {"month": {"$month": "$created"}, "year": {"$year": "$created"}}}, {"$group": {"_id": {"month": "$month", "year": "$year"}, "session_count": {"$sum": 1}}}]
    ...
    def _build_project_report(self, base_query):
        ...
        for p in projects:
            ...
            cont_query = {"projects": {"_id": {"project": p["_id"]}}, "sessions": {"project": p["_id"]}, "acquisitions": {"session": {"$in": session_ids}}, "analyses": {"parent.id": {"$in": parent_ids}}}
            ...
            for cont_name in ["projects", "sessions", "acquisitions", "analyses"]:
                ...
                pipeline = [{"$match": cont_query[cont_name]}, {"$unwind": "$files"}, {"$match": file_q}, {"$project": {"mbs": {"$divide": [{"$cond": ["$files.input", 0, "$files.size"]}, BYTES_IN_MEGABYTE]}}}, {"$group": {"_id": 1, "mb_total": {"$sum": "$mbs"}}}]
