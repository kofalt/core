...
def prefix(path, routes):
    ...
    return webapp2_extras.routes.PathPrefixRoute(path.format(**routing_regexes), routes)
...
endpoints = [
    ...
    prefix(
        ...
        [
            ...
            prefix("/gears", [route("/<:[^/]+>", GearHandler), route("/<:[^/]+>/invocation", GearHandler, h="get_invocation"), route("/<:[^/]+>/suggest/<:{cname}|subjects>/<:[^/]+>", GearHandler, h="suggest"), route("/<:[^/]+>/context/<:{cname}>/<:{oid}>", GearHandler, h="get_context", m=["GET"])]),
            # Batch jobs
            route("/batch", BatchHandler, h="get_all", m=["GET"]),
            route("/batch", BatchHandler, m=["POST"]),
            prefix("/batch", [route("/jobs", BatchHandler, h="post_with_jobs", m=["POST"]), route("/<:[^/]+>", BatchHandler, h="get", m=["GET"]), route("/<:[^/]+>/run", BatchHandler, h="run", m=["POST"]), route("/<:[^/]+>/cancel", BatchHandler, h="cancel", m=["POST"])]),
            # Devices
            ...
            prefix("/modalities", [route("/<modality_name:[^/]+>", ModalityHandler, m=["GET", "PUT", "DELETE"])]),
            # Site
            route("/site/providers", ProviderHandler, h="get_all", m=["GET"]),
            route("/site/providers", ProviderHandler, m=["POST"]),
            prefix("/site/providers", [route("/<_id:{oid}>", ProviderHandler, m=["GET", "PUT"]), route("/<_id:{oid}>/config", ProviderHandler, h="get_config", m=["GET"])]),
            ...
            # Data views
            prefix("/containers/<parent:{gid}|{oid}|{uid}>", [route("/views", DataViewHandler, h="list_views", m=["GET"]), route("/views", DataViewHandler, m=["POST"])]),
            prefix(
                "/views",
                [
            ...
            # Sessions
            prefix("/sessions", [route("/<cid:{oid}>/jobs", ContainerHandler, h="get_jobs", m=["GET"]), route("/<cid:{oid}>/subject", ContainerHandler, h="get_subject", m=["GET"])]),
            # Collections
            ...
            prefix("/<cont_name:collections|projects>", [prefix("/<cid:{oid}>", [route("/<list_name:permissions>", PermissionsListHandler, m=["POST"]), route("/<list_name:permissions>/<_id:{uid}>", PermissionsListHandler, m=["GET", "PUT", "DELETE"])])]),
            # Analyses
            route("/analyses/<_id:{oid}>", AnalysesHandler, m=["GET", "PUT"]),
            prefix("/analyses/<_id:{oid}>", [route("/files", AnalysesHandler, h="upload", m=["POST"]), route("/<filegroup:inputs|files>/<filename:{fname}>", AnalysesHandler, h="download", m=["GET"]), route("/info", AnalysesHandler, h="modify_info", m=["POST"])]),
            prefix("/<:{cname}>/<:{oid}>/<cont_name:analyses>/<cid:{oid}>", [route("/<list_name:notes>", NotesListHandler, m=["POST"]), route("/<list_name:notes>/<_id:{oid}>", NotesListHandler, name="notes", m=["GET", "PUT", "DELETE"])]),
            # Containers
                ...
                [
                    ...
                    prefix(
                        ...
                        [
                            ...
                            prefix("/analyses", [route("/<_id:{oid}>", AnalysesHandler, m=["GET", "PUT", "DELETE"]), route("/<_id:{oid}>/files", AnalysesHandler, h="upload", m=["POST"]), route("/<_id:{oid}>/<filegroup:inputs|files>/<filename:{fname}>", AnalysesHandler, h="download", m=["GET"])]),
            ...
            prefix("/subjects/master-code", [route("/<_id:[^/]+>", MasterSubjectCodeHandler, h="verify_code", m=["GET"])]),
        ],
    ),
]
