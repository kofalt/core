...
class RequestHandler(webapp2.RequestHandler):
    ...
    def initialization_auth(self):
        ...
        if session_token:
            ...
            self.device = config.db.devices.find_one_and_update({"label": label}, {"$set": {"label": label, "type": drone_method, "name": drone_name}}, upsert=True, return_document=pymongo.collection.ReturnDocument.AFTER)
        ...
        if self.origin["type"] == Origin.device:
            ...
            self.device = config.db.devices.find_one_and_update({"_id": self.origin["id"]}, {"$set": {"last_seen": datetime.datetime.utcnow(), "errors": []}}, return_document=pymongo.collection.ReturnDocument.AFTER)  # Reset errors list if device checks in
    ...
    def auth_status(self):
        ...
        """
        return {"origin": self.origin, "user_is_admin": self.user_is_admin, "is_device": self.origin["type"] == "device"}
    ...
    def pagination(self):
        ...
        pagination = {}
        parsers = {"after_id": util.parse_pagination_value, "filter": util.parse_pagination_filter_param, "sort": util.parse_pagination_sort_param}
    ...
    def log_user_access(self, access_type, cont_name=None, cont_id=None, filename=None, origin_override=None, download_ticket=None, job_id=None):
        origin = origin_override if origin_override is not None else self.origin

        try:
            log_user_access(self.request, access_type, cont_name=cont_name, cont_id=cont_id, filename=filename, origin=origin, download_ticket=download_ticket, job_id=job_id)
        except Exception as e:  # pylint: disable=broad-except
    ...
    def abort(self, code, detail=None, **kwargs):
        if isinstance(detail, jsonschema.ValidationError):
            detail = {"relative_path": list(detail.relative_path), "instance": detail.instance, "validator": detail.validator, "validator_value": detail.validator_value}
