...
class Placer(object):
    ...
    def save_file(self, file_attrs=None, ignore_hash_replace=False):
        ...
        if file_attrs is not None:
            ...
            container_before, self.container, saved_state = hierarchy.upsert_fileinfo(self.container_type, self.id_, file_attrs, self.access_logger, ignore_hash_replace=ignore_hash_replace, logger=self.logger)
            ...
            if saved_state == "ignored":
                ...
                rules.create_jobs(config.db, container_before, self.container, self.container_type, replaced_files=replaced_files, rule_failure_callback=self.handle_rule_failure)
    ...
    def update_file(self, file_attrs):
        ...
        if saved_state == "ignored":
            ...
            rules.create_jobs(config.db, container_before, self.container, self.container_type, rule_failure_callback=self.handle_rule_failure)
        ...
        if self.count == 0:
            ...
            for target in targets:
                ...
                for name in target[1]:
                    self.metadata_for_file[name] = {"container": target[0], "metadata": target[1][name]}
        self.count += 1
...
class PackfilePlacer(Placer):
    ...
    def finalize(self):
        ...
        self.zip_ = zipfile.ZipFile(config.local_fs.get_fs().open(tempZipPath, "wb"), "w", zipfile.ZIP_DEFLATED, allowZip64=True)
        ...
        for path in paths:
            ...
            # Report progress
            complete += 1
            yield encoder.json_sse_pack({"event": "progress", "data": {"done": complete, "total": total, "percent": (complete / float(total)) * 100}})
        ...
        self.origin = {"type": str(Origin.user), "id": uid}
        ...
        with config.local_fs.get_fs().open(tempZipPath, "rb") as (f1), config.primary_storage.open(token, util.path_from_uuid(token), "wb") as f2:
            while True:
        ...
        query = {"project": bson.ObjectId(self.p_id), "label": self.s_label, "group": self.g_id, "deleted": {"$exists": False}}

        # Updates if existing
        updates = util.mongo_dict({"permissions": self.permissions, "modified": self.timestamp})

        # Properties on insert
        insert_map = {"project": bson.ObjectId(self.p_id), "label": self.s_label, "group": self.g_id, "created": self.timestamp}
        ...
        if self.s_code or not session_exists:
            ...
            insert_map["parents"] = {"group": self.g_id, "project": bson.ObjectId(self.p_id), "subject": subject["_id"]}

        session = config.db.sessions.find_one_and_update(query, {"$set": updates, "$setOnInsert": insert_map}, upsert=True, return_document=pymongo.collection.ReturnDocument.AFTER)

        # Get or create an acquisition based on the hierarchy and provided labels.
        query = {"session": session["_id"], "label": self.a_label, "deleted": {"$exists": False}}
        ...
        acquisition = config.db.acquisitions.find_one_and_update(query, {"$set": updates, "$setOnInsert": insert_map}, upsert=True, return_document=pymongo.collection.ReturnDocument.AFTER)
        ...
        result = {"acquisition_id": str(acquisition["_id"]), "session_id": str(session["_id"]), "info": cgi_attrs}

        # Report result
        yield encoder.json_sse_pack({"event": "result", "data": result})
...
class GearPlacer(Placer):
    ...
    def process_file_field(self, file_attrs):
        if self.metadata:
            ...
            self.metadata.update({"exchange": {"rootfs-hash": proper_hash, "git-commit": "local", "rootfs-url": "INVALID", "rootfs-id": file_attrs["_id"]}})
