    ...
    def post(self):
        ...
        while True:
            if use_patient_id:
                ...
                found_subj_codes = self.mapper.find(first_name=payload["first_name"], last_name=payload["last_name"], date_of_birth=payload["date_of_birth"])
            found_subj_codes = list(found_subj_codes)

            if len(found_subj_codes) > 1:
                raise errors.InputValidationException("Found multiple master subject codes. " "Use `patient_id` and set `use_patient_id` to true in the payload " "to uniquely identify the subject.")
                ...
                try:
                    ...
                    new_subj_code = code_generator.generate_code(length=code_length, allowed_chars=code_allowed_chars, prefix=code_prefix)
                    ...
                    if time.time() - start > RETRY_CODE_GENERATION_TILL:
                        self.log.critical("Couldn't generate new master subject code in {} second(s). " "Possibly we are running out of the free ones. Consider increasing the size of the code.".format(time.time() - start))
                        raise
