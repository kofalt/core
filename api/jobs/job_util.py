        ...
        if job.config:
            ...
            for config_input in config_inputs.values():
                if config_input.get("base") == "file":
                    ...
                    handler.log_user_access(AccessType.view_file, cont_name=file_parent_type, cont_id=file_parent_id, filename=config_input["location"].get("name"))
        if job.produced_metadata:
            for container_type, metadata in job.produced_metadata.items():
                if job.parents.get(container_type):
                    handler.log_user_access(AccessType.view_container, cont_name=container_type, cont_id=job.parents[container_type])
                if container_type == "session" and metadata.get("subject"):
                    if job.parents.get("subject"):
                        handler.log_user_access(AccessType.view_subject, cont_name="subject", cont_id=job.parents["subject"])
    except (TypeError, KeyError, AttributeError) as e:
    ...
    for parent in parent_tree:
        if not uid or has_access(uid, parent, "ro"):
            ...
            for key, value in parent_context.items():
                context[key] = {"container_type": cont_type, "label": parent["label"], "id": parent["_id"], "value": value}
...
def resolve_context_inputs(config, gear, cont_type, cont_id, uid, context=None):
    ...
    for x in gear["gear"]["inputs"]:
        ...
        if input_type == "context":
            ...
            if x in context:
                config["inputs"][x] = {"base": "context", "found": True, "value": context[x]["value"]}
            else:
                config["inputs"][x] = {"base": "context", "found": False}
