def test_signed_url_reaper_upload(as_drone, mocker):
    ...
    payload = {"metadata": {"group": {"_id": "scitran"}, "project": {"label": ""}, "session": {"uid": "session_uid", "subject": {"code": "bela"}}, "acquisition": {"uid": "acquisition_uid", "files": [{"name": "test"}, {"name": "test2"}]}}, "filenames": ["test", "test2"]}

    r = as_drone.post("/upload/reaper?ticket=", json=payload)
...
def test_signed_url_label_upload(as_drone, data_builder, mocker):
    ...
    payload = {"metadata": {"group": {"_id": group}, "project": {"label": "test_project", "files": [{"name": "project.csv"}]}}, "filenames": ["project.csv"]}

    r = as_drone.post("/upload/label?ticket=", json=payload)
...
def test_signed_url_engine_upload(as_drone, data_builder, mocker):
    ...
    payload = {"metadata": {"project": {"label": "engine project", "info": {"test": "p"}, "files": [{"name": "one.csv", "type": "engine type 0", "info": {"test": "f0"}}]}}, "filenames": ["one.csv"]}

    r = as_drone.post("/engine?upload_ticket=&level=%s&id=%s" % ("project", project), json=payload)
...
def test_signed_url_analysis_engine_upload(data_builder, file_form, as_drone, mocker):
    ...
    body = file_form("one.csv", meta={"label": "test analysis", "inputs": [{"name": "one.csv"}]})
    # create acquisition analysis
    r = as_drone.post("/sessions/" + session + "/analyses", POST=body)
    assert r.ok
    session_analysis = r.json["_id"]

    payload = {"metadata": {"type": "text", "value": {"label": "test"}, "enabled": True}, "filenames": ["one.csv"]}

    r = as_drone.post("/engine?upload_ticket=&level=%s&id=%s" % ("analysis", session_analysis), json=payload)
...
def test_signed_url_filelisthandler_upload(as_drone, data_builder, mocker):
    ...
    payload = {"metadata": {}, "filenames": ["one.csv"]}
