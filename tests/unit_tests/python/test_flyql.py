...
def test_flyql_parser():
    ...
    assert parse(r"subject.sex == male and subject.age < 37") == And(Term("==", "subject.sex", "male"), Term("<", "subject.age", "37"))

    assert parse(r"not (subject.sex == male and subject.age < 37)") == Not(Group(And(Term("==", "subject.sex", "male"), Term("<", "subject.age", "37"))))

    assert parse(r"not subject.label contains 666") == Not(Term("contains", "subject.label", "666"))

    assert parse(r"not subject.race exists AND subject.sex exists") == And(Not(Term("exists", "subject.race", True)), Term("exists", "subject.sex", True))

    # Operator precedence
    assert parse(r'not a == b or c == "d"') == Or(Not(Term("==", "a", "b")), Term("==", "c", "d"))

    assert parse(r"a == b or c < d and e =~ f") == Or(Term("==", "a", "b"), And(Term("<", "c", "d"), Term("=~", "e", "f")))

    assert parse(r"a < b and c < d or e < f") == Or(And(Term("<", "a", "b"), Term("<", "c", "d")), Term("<", "e", "f"))
    ...
    assert repr(tree) == ("And(lhs=Group(Or(lhs=Term(op='=' field='a' phrase='b') " "rhs=Term(op='<' field='c' phrase='d'))) rhs=Not(Term(op='>' field='e' phrase='f')))")
...
def test_flyql_to_elastic():
    ...
    # And operator
    assert to_elastic(r"subject.age exists AND subject.code like ex%") == {"bool": {"must": [{"exists": {"field": "subject.age"}}, {"wildcard": {"subject.code.raw": "ex*"}}]}}

    # Or operator
    assert to_elastic(r"subject.age exists OR subject.code like ex%") == {"bool": {"should": [{"exists": {"field": "subject.age"}}, {"wildcard": {"subject.code.raw": "ex*"}}]}}

    # Group operator
    assert to_elastic(r"(subject.age = 32)") == {"bool": {"must": [{"term": {"subject.age": 32}}]}}

    # Nested logic
    assert to_elastic(r"a = b and c = d or e = f") == {"bool": {"should": [{"bool": {"must": [{"term": {"a.raw": "b"}}, {"term": {"c.raw": "d"}}]}}, {"term": {"e.raw": "f"}}]}}

    assert to_elastic(r"a = b and (c = d or not e = f)") == {"bool": {"must": [{"term": {"a.raw": "b"}}, {"bool": {"should": [{"term": {"c.raw": "d"}}, {"bool": {"must_not": [{"term": {"e.raw": "f"}}]}}]}}]}}
