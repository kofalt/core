...
def test_master_subject_code_handler(as_admin, mocker, api_db):
    ...
    mock_gen_code = mocker.patch(
        'api.master_subject_code.handlers.master_subject_code_handler.code_generator.generate_code')
    mock_gen_code.return_value = 'fw-BCD123'
    mocker.patch(
        'api.master_subject_code.handlers.master_subject_code_handler.RETRY_CODE_GENERATION_TILL',
        0)

    resp = as_admin.post('/subjects/master-code', json={
        'use_patient_id': True,
        'patient_id': 'MRN-ABC123'
    })
    ...
    mocker.patch(
        'api.master_subject_code.handlers.master_subject_code_handler.config.get_item',
        new=mock_get_item)

    as_admin.post('/subjects/master-code', json={
        'use_patient_id': True,
        'patient_id': 'MRN-ABC567'
    })
...
def test_verify_master_subject_code(mocker, as_user):
    def mock_get_item(outer, inner):
        dumped_config = json.dumps({
            'url': 'http://localhost:8080/api/subjects/master-code',
            'headers': dict(as_user.headers)
        })
        _config = {
            'master_subject_code': {
                'verify_config': dumped_config
            }
        }
        return _config[outer][inner]
    ...
    mocked_requests.assert_called_with(
        'http://localhost:8080/api/subjects/master-code/CODE',
