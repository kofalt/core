...
def test_search(as_public, as_drone, es, as_user):
    ...
    es.search.return_value = {"aggregations": {"by_container": {"buckets": [{"by_top_hit": {"hits": {"hits": [results]}}}]}}}
    r = as_drone.post("/dataexplorer/search", json={"return_type": cont_type, "all_data": True, "search_string": search_str, "filters": [{"terms": {filter_key: filter_value}}, {"range": filter_range}]})
    es.search.assert_called_with(
        body={
            "size": 0,
            "query": {"bool": {"must": {"match": {"_all": "search"}}, "filter": {"bool": {"must": [{"terms": {filter_key + ".raw": filter_value}}, {"range": filter_range}, {"term": {"deleted": False}}]}}}},
            "aggs": {"by_container": {"terms": {"field": cont_type + "._id", "size": 100}, "aggs": {"by_top_hit": {"top_hits": {"_source": deh.SOURCE[cont_type], "size": 1}}}}},
        },
        doc_type="flywheel",
        index="data_explorer",
    )
    ...
    es.search.assert_called_with(
        body={"size": 0, "query": {"bool": {"filter": {"bool": {"must": [{"term": {"deleted": False}}]}}}}, "aggs": {"by_container": {"terms": {"field": cont_type + "._id", "size": 100}, "aggs": {"by_top_hit": {"top_hits": {"_source": deh.SOURCE[cont_type], "size": 1}}}}}},
        doc_type="flywheel",
        index="data_explorer",
    )
    assert r.ok
    ...
    r = as_drone.post("/dataexplorer/search", json={"return_type": cont_type, "all_data": True})
    es.search.assert_called_with(
        body={"_source": deh.SOURCE[cont_type], "query": {"bool": {"filter": {"bool": {"must": [{"term": {"container_type": cont_type}}, {"term": {"deleted": False}}]}}}}, "script_fields": {"info_exists": deh.INFO_EXISTS_SCRIPT}, "size": 100}, doc_type="flywheel", index="data_explorer"
    )
    assert r.ok
    ...
    es.search.assert_called_with(
        body={
            ...
            "query": {"bool": {"must": {"match": {"_all": search_str}}, "filter": {"bool": {"must": [{"term": {"container_type": cont_type}}, {"terms": {filter_key + ".raw": filter_value}}, {"range": filter_range}, {"term": {"deleted": False}}]}}}},
    ...
    es.search.return_value = {"hits": {"hits": copy.deepcopy(raw_file_results)}}
    r = as_drone.post("/dataexplorer/search", json={"return_type": cont_type, "all_data": False, "search_string": search_str, "filters": [{"terms": {filter_key: filter_value}}, {"range": filter_range}]})
    es.search.assert_called_with(
        body={
            "query": {"bool": {"filter": {"bool": {"must": [{"term": {"container_type": cont_type}}, {"terms": {filter_key + ".raw": filter_value}}, {"range": filter_range}, {"term": {"permissions._id": None}}, {"term": {"deleted": False}}]}}, "must": {"match": {"_all": search_str}}}},
            "_source": deh.SOURCE[cont_type],
            "script_fields": {"info_exists": deh.INFO_EXISTS_SCRIPT},
            "size": 100,
        },
    ...
    r = as_user.post("/dataexplorer/search", json={"return_type": cont_type, "all_data": True, "search_string": search_str, "filters": [{"terms": {filter_key: filter_value}}, {"range": filter_range}]})
    assert r.status_code == 403

    # file search w/ search null filter
    es.search.return_value = {"hits": {"hits": copy.deepcopy(raw_file_results)}}
    r = as_drone.post("/dataexplorer/search", json={"return_type": cont_type, "all_data": True, "filters": [{"terms": {filter_key: [filter_value, "null"]}}]})
    es.search.assert_called_with(
        body={
            "_source": deh.SOURCE[cont_type],
            "query": {"bool": {"filter": {"bool": {"must": [{"term": {"container_type": cont_type}}, {"bool": {"should": [{"bool": {"must": [{"bool": {"must_not": [{"exists": {"field": filter_key}}]}}]}}, {"terms": {filter_key + ".raw": [filter_value]}}]}}, {"term": {"deleted": False}}]}}}},
            "script_fields": {"info_exists": deh.INFO_EXISTS_SCRIPT},
            "size": 100,
        },
        doc_type="flywheel",
    ...
    # file search w/ search string and filter
    es.search.return_value = {"hits": {"total": 0, "max_score": 0, "hits": []}, "aggregations": {"count": {"value": 0}}}
    r = as_drone.post("/dataexplorer/search", json={"return_type": cont_type, "all_data": True, "filters": [{"terms": {filter_key: filter_value}}], "size": "all"})
    es.search.assert_called_with(
        body={"_source": deh.SOURCE[cont_type], "query": {"bool": {"filter": {"bool": {"must": [{"term": {"container_type": cont_type}}, {"terms": {filter_key + ".raw": filter_value}}, {"term": {"deleted": False}}]}}}}, "script_fields": {"info_exists": deh.INFO_EXISTS_SCRIPT}, "size": 1},
        doc_type="flywheel",
        index="data_explorer",
    )
    assert r.ok
    ...
    r = as_drone.post("/dataexplorer/search", json={"return_type": cont_type, "all_data": True, "filters": [{"terms": {filter_key: filter_value}}], "size": "10000000"})
    assert r.status_code == 400

    # Search for training set given a set of nodes
    r = as_drone.post("/dataexplorer/search/training", json={"labels": ["file.classifications"], "files": [{"parent_id": "123456789009876543211234", "name": "example.txt"}], "output": {"type": "project", "id": "000000000000000000000000"}})
    print(r.json)
    es.mget.assert_called_with(body={"docs": [{"_source": list(set(deh.SOURCE_FILE + ["file.classifications"])), "_id": "123456789009876543211234_example.txt"}]}, index="data_explorer")
    assert r.status_code == 404  # the output container doesn't exist

    # Query search
    r = as_drone.post("/dataexplorer/search/training", json={"labels": ["file.classifications"], "search_query": {"return_type": cont_type, "all_data": True}, "output": {"type": "project", "id": "000000000000000000000000"}})

    raw_file_results = [{"fields": {"info_exists": [True]}, "_source": {"file": {}}}]
    es.search.return_value = {"hits": {"hits": copy.deepcopy(raw_file_results)}}

    es.search.assert_called_with(
        body={"_source": list(set(deh.SOURCE_FILE + ["file.classifications"])), "query": {"bool": {"filter": {"bool": {"must": [{"term": {"container_type": cont_type}}, {"term": {"deleted": False}}]}}}}, "script_fields": {"info_exists": deh.INFO_EXISTS_SCRIPT}, "size": 100},
        doc_type="flywheel",
        index="data_explorer",
    )
...
def test_get_facets(as_public, as_drone, es):
    ...
    session_age = "test"
    es.search.return_value = {"aggregations": {"session_age": {"session.age": session_age}, "by_session": {}}}
...
def test_search_fields(as_public, as_drone, es):
    ...
    es.search.assert_called_with(body={"size": 15, "query": {"match": {"name": query_field}}}, doc_type="flywheel_field", index="data_explorer_fields")
    assert r.ok
...
def test_index_fields(as_public, as_drone, es):
    ...
    def es_indices_exists(index):
        return index in indices

    def es_indices_create(index=None, body=None):
        indices.add(index)

    def es_indices_delete(index):
    ...
    r = as_drone.post("/dataexplorer/index/fields?hard-reset=true")
    es.indices.create.assert_called_with(index="data_explorer_fields", body={"settings": {"number_of_shards": 1, "number_of_replicas": 0, "analysis": deh.ANALYSIS}, "mappings": {"_default_": {"_all": {"enabled": True}, "dynamic_templates": deh.DYNAMIC_TEMPLATES}, "flywheel": {}}})
    assert r.ok
    ...
    es.indices.get_mapping.return_value = {"data_explorer": {"mappings": {"flywheel": {"properties": fields}}}}
    es.search.return_value = {"aggregations": {"results": {"sum_other_doc_count": 0, "buckets": [{"doc_count": 0}]}}}
    es.index.reset_mock()
    r = as_drone.post("/dataexplorer/index/fields")
    for field_name in fields:
        ...
        if field_type == "string":
            es.search.assert_any_call(body={"aggs": {"results": {"terms": {"field": field_name + ".raw", "size": 15}}}, "size": 0}, doc_type="flywheel", index="data_explorer")
        es.index.assert_any_call(body=json.dumps({"name": field_name, "type": field_type, "facet": False}), doc_type="flywheel_field", id=field_name, index="data_explorer_fields")
    assert r.ok
...
def test_aggregate_field_values(as_public, as_drone, es):
    ...
    r = as_drone.post("/dataexplorer/search/fields/aggregate", json={"field_name": field_name})
    es.search.assert_called_with(body={"aggs": {"results": {"terms": {"field": field_name + ".raw", "size": 15, "missing": "null"}}}, "query": {"bool": {"filter": [{"term": {"deleted": False}}], "must": {"match_all": {}}}}, "size": 0}, doc_type="flywheel", index="data_explorer")
    assert r.ok
    ...
    es.search.assert_called_with(body={"aggs": {"results": {"terms": {"field": field_name + ".raw", "size": 15, "missing": "null"}}}, "query": {"bool": {"filter": [{"term": {"deleted": False}}], "must": {"match": {"field": search_str}}}}, "size": 0}, doc_type="flywheel", index="data_explorer")
    assert r.ok
    assert r.json == result
    ...
    r = as_drone.post("/dataexplorer/search/fields/aggregate", json={"field_name": field_name})
    es.search.assert_called_with(body={"aggs": {"results": {"stats": {"field": field_name}}}, "query": {"bool": {"filter": [{"term": {"deleted": False}}], "must": {"match_all": {}}}}, "size": 0}, doc_type="flywheel", index="data_explorer")
    assert r.ok
    assert r.json == result

    # get typeahead w/ search string for integer|float|date field type
    r = as_drone.post("/dataexplorer/search/fields/aggregate", json={"field_name": field_name, "search_string": search_str})
    es.search.assert_called_with(body={"aggs": {"results": {"stats": {"field": field_name}}}, "query": {"bool": {"filter": [{"term": {"deleted": False}}], "must": {"match": {"field": search_str}}}}, "size": 0}, doc_type="flywheel", index="data_explorer")
    assert r.ok
    assert r.json == result
...
def test_structured_search(as_public, as_drone, es, as_user):
    ...
    es.search.assert_called_with(
        body={"size": 0, "query": {"bool": {"filter": {"bool": {"must": [query_es, {"term": {"deleted": False}}]}}}}, "aggs": {"by_container": {"terms": {"field": cont_type + "._id", "size": 100}, "aggs": {"by_top_hit": {"top_hits": {"_source": deh.SOURCE[cont_type], "size": 1}}}}}},
        doc_type="flywheel",
        index="data_explorer",
    )
    assert r.ok
    ...
    search_str = "search"
    es.search.return_value = {"aggregations": {"by_container": {"buckets": [{"by_top_hit": {"hits": {"hits": [results]}}}]}}}
    r = as_drone.post("/dataexplorer/search", json={"return_type": cont_type, "all_data": True, "search_string": search_str, "structured_query": query})
    es.search.assert_called_with(
        body={
            "size": 0,
            "query": {"bool": {"must": {"match": {"_all": search_str}}, "filter": {"bool": {"must": [query_es, {"term": {"deleted": False}}]}}}},
            "aggs": {"by_container": {"terms": {"field": cont_type + "._id", "size": 100}, "aggs": {"by_top_hit": {"top_hits": {"_source": deh.SOURCE[cont_type], "size": 1}}}}},
        },
        doc_type="flywheel",
        index="data_explorer",
...
def test_search_suggest(as_public, as_drone, es, as_user):
    ...
    es.search.assert_called_with(body={"size": 15, "query": {"match": {"name": "subject."}}}, doc_type="flywheel_field", index="data_explorer_fields")
    assert r.ok
    assert r.json == {"from": 0, "suggestions": [{"display": result_source, "value": result_source}]}
    ...
    expected_results = [{"display": "value_result", "value": "value_result"}, {"display": 'value "result"', "value": '"value \\"result\\""'}]
    es.get.return_value = {"_source": {"type": "string"}}
    es.search.return_value = {"aggregations": {"results": results}}
    r = as_drone.post("/dataexplorer/search/suggest", json={"structured_query": query})
    es.search.assert_called_with(body={"aggs": {"results": {"terms": {"field": "subject.code.raw", "size": 15, "missing": "null"}}}, "query": {"bool": {"filter": [{"term": {"deleted": False}}], "must": {"match": {"subject.code": "ex"}}}}, "size": 0}, doc_type="flywheel", index="data_explorer")
    assert r.ok
    assert r.json == {"from": 16, "suggestions": expected_results}
