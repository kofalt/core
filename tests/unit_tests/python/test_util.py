...
def test_mongo_sanitize_fields():
    ...
    input_fields = {1: 1, 2.0: 2.0, "foo.bar$baz": "foo.bar$baz", "list": [1, 2.0, "foo.bar$baz", obj], "obj": obj}

    expected_fields = {"1": 1, "2_0": 2.0, "foo_bar-baz": "foo.bar$baz", "list": [1, 2.0, "foo_bar-baz", obj], "obj": obj}

    assert util.mongo_sanitize_fields(input_fields) == expected_fields


def test_deep_update():
    d = {"old": 1, "both": 1, "dict": {"old": 1, "both": 1}}

    u = {"both": 2, "new": 2, "dict": {"both": 2, "new": 2}}

    util.deep_update(d, u)

    assert d == {"old": 1, "both": 2, "new": 2, "dict": {"old": 1, "both": 2, "new": 2}}


def test_enum():
    # create test enum class
    TestEnum = util.Enum("TestEnum", {"foo": 1, "bar": 2})
