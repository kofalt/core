...
def test_multiproject_get_provider_id_for_container(mock_cs_factory, MockSiteSettingsCls):
    ...
    container = {"_id": "test", "parents": {"group": bson.ObjectId(), "project": bson.ObjectId()}}
    ...
    MockContainerStorage.get_el.side_effect = ({"_id": "project"}, {"_id": "group"})
    ...
    # == Group ==
    MockContainerStorage.get_el.side_effect = ({"_id": "project"}, {"_id": "group", "providers": {"compute": group_provider_id}})
    is_site, provider_id = picker.get_provider_id_for_container(container, cls)
    assert is_site == False
    assert provider_id == group_provider_id

    # == Project ==
    MockContainerStorage.get_el.side_effect = ({"_id": "project", "providers": {"compute": project_provider_id}}, {"_id": "group", "providers": {"compute": group_provider_id}})
    is_site, provider_id = picker.get_provider_id_for_container(container, cls)
    assert is_site == False
...
def test_multiproject_get_compute_provider_id_for_job(mock_cs_factory, MockSiteSettingsCls):
    ...
    destination = {"_id": "test", "parents": {"group": bson.ObjectId(), "project": bson.ObjectId()}}
