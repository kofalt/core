...
def test_jwt_auth(config, as_drone, as_public, api_db):
    ...
    config["auth"]["ldap"] = dict(verify_endpoint="http://ldap.test", check_ssl=False)
    ...
    with requests_mock.Mocker() as m:
        ...
        assert as_drone.post("/users", json={"_id": uid, "disabled": True, "firstname": "test", "lastname": "test"}).ok
        r = as_public.post("/login", json={"auth_type": "ldap", "code": "test"})
        assert r.status_code == 402
...
def test_jwt_auth_with_mail_format(config, as_drone, as_public, api_db):
    # inject ldap (jwt) auth config
    config["auth"]["ldap"] = dict(verify_endpoint="http://ldap.test", mail_format="{uid}@scitran.edu", check_ssl=False)

    uid = "ldapuser"
    email = "ldapuser@scitran.edu"
    api_db.users.delete_one({"_id": email})
    with requests_mock.Mocker() as m:
        # try to log in w/ ldap - pretend provider doesn't return uid
        m.post(config.auth.ldap.verify_endpoint, json={"mail": "ldap@ldap.test"})
        r = as_public.post("/login", json={"auth_type": "ldap", "code": "test"})
        assert r.status_code == 401
...
def test_cas_auth(config, as_drone, as_public, api_db):
    ...
    config["auth"]["cas"] = dict(service_url_state="?state=cas", auth_endpoint="http://cas.test/cas/login", verify_endpoint="http://cas.test/cas/serviceValidate", namespace="cas.test", display_string="CAS Auth")
...
def test_google_auth(config, as_drone, as_public, api_db):
    ...
    with requests_mock.Mocker() as m:
        ...
        as_drone.post("/users", json={"_id": uid, "disabled": True, "firstname": "test", "lastname": "test"}).ok
        r = as_public.post("/login", json={"auth_type": "google", "code": "test"})
        assert r.status_code == 402
        ...
        m.head(re.compile("https://gravatar.com/avatar"))
        m.post(config.auth.google.token_endpoint, json={"access_token": "test", "expires_in": 60, "refresh_token": "test"})
        r = as_public.post("/login", json={"auth_type": "google", "code": "test"})
        assert r.ok
        assert "gravatar" in api_db.users.find_one({"_id": uid})["avatars"]
        ...
        api_db.authtokens.update_one({"_id": token_1}, {"$set": {"expires": datetime.datetime.now() - datetime.timedelta(seconds=1)}})
        ...
        m.post(config.auth.google.token_endpoint, json={"access_token": "test", "expires_in": -1, "refresh_token": "test"})
        r = as_public.post("/login", json={"auth_type": "google", "code": "test"})
        ...
        api_db.authtokens.update_one({"_id": token_2}, {"$set": {"expires": datetime.datetime.now() - datetime.timedelta(seconds=1)}})
...
def test_wechat_auth(config, as_drone, as_public, api_db):
    ...
    config["auth"]["wechat"] = dict(client_id="test", client_secret="test", token_endpoint="http://wechat.test/token", refresh_endpoint="http://wechat.test/refresh")
    ...
    with requests_mock.Mocker() as m:
        ...
        assert as_drone.post("/users", json={"_id": uid, "firstname": "test", "lastname": "test"}).ok
        r = as_public.post("/login", json={"auth_type": "wechat", "code": "test"})
        assert r.status_code == 402

        # try to log in w/ wechat w/ invalid regitration code
        r = as_public.post("/login", json={"auth_type": "wechat", "code": "test", "registration_code": "test"})
        assert r.status_code == 402
        ...
        r = as_public.post("/login", json={"auth_type": "wechat", "code": "test", "registration_code": regcode_1})
        assert r.status_code == 401
        ...
        r = as_public.post("/login", json={"auth_type": "wechat", "code": "test", "registration_code": regcode_2})
        assert r.status_code == 402

        # log in w/ wechat
        m.post(config.auth.wechat.token_endpoint, json={"openid": "test", "refresh_token": "test", "access_token": "test", "expires_in": 60})
        ...
        # access api w/ expired token - test refresh
        m.post(config.auth.wechat.token_endpoint, json={"openid": "test", "refresh_token": "test", "access_token": "test", "expires_in": -1})
        r = as_public.post("/login", json={"auth_type": "wechat", "code": "test"})
        token_2 = r.json["token"]
...
def test_saml_auth(config, as_drone, as_public, api_db):
    ...
    config["auth"]["saml"] = dict(auth_endpoint="http://saml.test/secure/login/saml", verify_endpoint="http://saml.test/Shibboleth.sso/Session", refresh_rate=300, display_string="SAML Auth", uid_key_name="mail")
