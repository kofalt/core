...
def test_apply_env_variables(mocker, tmpdir):
    ...
    config = {"auth": {"initial": "auth"}, "test": {"true": "", "false": "", "none": ""}, "site": {"upload_maximum_bytes": "10737418240"}, "features": {"test": True, "true": False}}

    expected_features = {"test": True, "true": True, "false": False, "none": "none", "subjects_view": "why not"}

    api.config.apply_env_variables(config)
    assert config == {"auth": {"test": "test"}, "test": {"true": True, "false": False, "none": None}, "site": {"upload_maximum_bytes": "10"}, "features": expected_features}
    ...
    api.config.apply_env_variables(config)
    assert config == {"auth": {"test2": "test2"}, "test": {"true": True, "false": False, "none": None}, "site": {"upload_maximum_bytes": "10"}, "features": expected_features}
    ...
    db.singletons.find_one.return_value = {"core": {"debug": False, "log_level": "info"}, "auth": {"test": "test"}, "test": {"true": True, "false": False, "none": None}, "site": {"upload_maximum_bytes": "10"}, "features": features2}
