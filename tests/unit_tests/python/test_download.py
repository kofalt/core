...
def test_filtered_files():
    cont = {
        "inputs": [{"name": "test.dcm", "type": "dicom"}],
        "files": [{"name": "test.csv", "type": "csv", "tags": ["foo"]}, {"name": "test2.csv", "type": "csv", "tags": ["bar"]}, {"name": "deleted.csv", "type": "csv", "deleted": datetime.datetime.now()}, {"name": "test.nii", "type": "nifti", "tags": ["foo", "bar"]}],
    }
...
def test_archive_stream(mocker, data_builder, file_form, as_drone, randstr):
    ...
    session = data_builder.create_session(label="session1", project=project, subject={"code": randstr(), "master_code": randstr()})
    session2 = data_builder.create_session(label="session1", project=project, subject={"code": randstr(), "master_code": randstr()})
    ...
    file_name = "test.csv"
    as_drone.post("/acquisitions/" + acquisition + "/files", POST=file_form(file_name, meta={"name": file_name, "type": "csv"}))

    as_drone.post("/acquisitions/" + acquisition2 + "/files", POST=file_form(file_name, meta={"name": file_name, "type": "csv"}))

    r = as_drone.post("/download", json={"optional": False, "nodes": [{"level": "acquisition", "_id": acquisition}, {"level": "acquisition", "_id": acquisition2}]})
    assert r.ok
