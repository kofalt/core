...
def site_providers(api_db, data_builder):
    ...
    # Set site providers
    providers = {"compute": data_builder.get_or_create("compute_provider")}
...
def legacy_cas_file(as_admin, api_db, data_builder, randstr, file_form):
    ...
    file_info = api_db["projects"].find_one({"files.name": file_name})["files"][0]
    file_id = file_info["_id"]
    file_hash = file_info["hash"]
    # verify cas backward compatibility
    api_db["projects"].find_one_and_update({"files.name": file_name}, {"$unset": {"files.$._id": ""}})
