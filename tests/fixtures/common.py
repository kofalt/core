    ...
    return attrdict.AttrDict(
        {
            ...
            "collection": {},
            "gear": {
                "exchange": {"git-commit": "aex", "rootfs-hash": "sha384:oy", "rootfs-url": "https://test.test"},
                "gear": {
                    "author": "test",
                    "config": {},
                    "description": "test",
                    "inputs": {"text": {"base": "file", "name": {"pattern": "^.*.txt$"}, "size": {"maximum": 100000}}},
                    "label": "test",
            ...
            "job": {"inputs": {}},
            "compute_provider": {"provider_class": "compute", "provider_type": "static", "config": {}},
            "storage_provider": {"provider_class": "storage", "provider_type": "static", "config": {}},
        }
    )
...
def as_drone(session):
    ...
    _session = session()
    _session.headers.update({"X-SciTran-Method": "bootstrapper", "X-SciTran-Name": "Bootstrapper", "X-SciTran-Auth": SCITRAN_CORE_DRONE_SECRET})
    return _session
...
class DataBuilder(object):
    child_to_parent = {"project": "group", "session": "project", "acquisition": "session"}
    ...
    def create(self, resource, **kwargs):
        ...
        if resource == "group":
            ...
            if "providers" not in payload:
                ...
                payload["providers"] = {"compute": self.get_or_create("compute_provider")}
        ...
        if callable(r.json):
            ...
            r_json = r.json
        if not r.ok:
            raise Exception("DataBuilder failed to create {}: {}\n" "Payload was:\n{}".format(resource, r_json["message"], payload))
        _id = r_json["_id"]

        # inject api key if it was provided
        if resource == "user" and user_api_key:
            self.api_db.apikeys.insert_one({"_id": user_api_key, "created": datetime.datetime.utcnow(), "last_seen": None, "type": "user", "origin": {"type": "user", "id": _id}})
