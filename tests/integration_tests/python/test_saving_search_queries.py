def test_saving_search_queries(as_admin, data_builder, as_user):
    ...
    # Save a search
    r = as_admin.post("/dataexplorer/queries", json={"label": "search1", "search": {"return_type": "session"}, "parent": {"type": "project", "id": project}})
    assert r.ok
    search = r.json()["_id"]
...
def test_saving_site_search_queries(as_admin, data_builder, as_user, as_public):
    ...
    r = as_user.post("/dataexplorer/queries", json={"label": "search1", "search": {"return_type": "session"}, "parent": {"type": "site", "id": "site"}})
    assert r.status_code == 403
    ...
    # Save a search
    r = as_admin.post("/dataexplorer/queries", json={"label": "search1", "search": {"return_type": "session"}, "parent": {"type": "site", "id": "site"}})
    assert r.ok
    search = r.json()["_id"]
