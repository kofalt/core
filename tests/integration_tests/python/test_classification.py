...
def test_modalities(data_builder, as_admin, as_user, api_db):
    ...
    payload = {"_id": "MR", "classification": {"Intent": ["Structural", "Functional", "Localizer"], "Measurement": ["B0", "B1", "T1", "T2"]}}
    ...
    update = {"classification": {"Intent": ["new", "stuff"]}}
...
def test_edit_file_classification(data_builder, as_admin, as_user, file_form, api_db):
    ...
    r = as_admin.put("/projects/" + project + "/files/" + file_name, json={"modality": "MR"})

    api_db.projects.update({"_id": bson.ObjectId(project), "files": {"$elemMatch": {"name": file_name}}}, {"$set": {"files.$.measurements": ["anatomy_t1w"]}})
    ...
    r = as_admin.post("/projects/" + project + "/files/" + file_name + "/classification", json={"delete": ["this", "is"], "replace": {"not_going": "to_happen"}})
    assert r.status_code == 400

    # Send improper payload
    r = as_admin.post("/projects/" + project + "/files/" + file_name + "/classification", json={"delete": ["should", "be", "a", "map"]})
    assert r.status_code == 400

    # Send improper payload
    r = as_admin.post("/projects/" + project + "/files/" + file_name + "/classification", json={"set": "cannot do this"})
    assert r.status_code == 400

    # Attempt full replace of classification
    file_cls = {"Intent": ["Structural"], "Measurement": ["B1", "T1"], "Custom": ["Custom Value"]}

    r = as_admin.post("/projects/" + project + "/files/" + file_name + "/classification", json={"replace": file_cls})
    assert r.ok
    ...
    r = as_admin.post("/projects/" + project + "/files/" + file_name + "/classification", json={"add": {"Intent": ["Functional"]}})
    assert r.ok
    ...
    r = as_admin.post("/projects/" + project + "/files/" + file_name + "/classification", json={"delete": {"Intent": ["Structural"], "Measurement": ["B1"]}})
    assert r.ok
    ...
    r = as_admin.post("/projects/" + project + "/files/" + file_name + "/classification", json={"add": {"Intent": ["Localizer"]}, "delete": {"Intent": ["Functional"]}})
    assert r.ok
    ...
    r = as_admin.post("/projects/" + project + "/files/" + file_name + "/classification", json={"delete": {"Intent": ["Structural", "Functional"]}})
    assert r.ok
    ...
    r = as_admin.post("/projects/" + project + "/files/" + file_name + "/classification", json={"add": {"Intent": ["Localizer"]}})
    assert r.ok
    ...
    r = as_admin.post("/projects/" + project + "/files/" + file_name + "/classification", json={"add": {"Measurement": ["t2", "b0"], "custom": ["lowercase"]}})
    assert r.ok
    ...
    # Ensure lowercase gets formatted in correct format via modality's classification
    r = as_admin.post("/projects/" + project + "/files/" + file_name + "/classification", json={"delete": {"Measurement": ["t2"], "custom": ["lowercase"]}})
    assert r.ok
    ...
    r = as_admin.post("/projects/" + project + "/files/" + file_name + "/classification", json={"replace": {"made-up": ["fake"], "Intent": ["not real"]}})
    assert r.status_code == 422
    assert r.json()["unaccepted_keys"] == ["made-up:fake", "Intent:not real"]

    # Use 'replace' to set file classification to {}
    r = as_admin.post("/projects/" + project + "/files/" + file_name + "/classification", json={"replace": {}})
    assert r.ok
    ...
    r = as_admin.put("/projects/" + project + "/files/" + file_name, json={"modality": "new unknown"})
    ...
    r = as_admin.post("/projects/" + project + "/files/" + file_name + "/classification", json={"modality": modality1, "add": {"Intent": ["Functional"]}})
    assert r.status_code == 400

    # Update modality and replace classification at the same time
    r = as_admin.post("/projects/" + project + "/files/" + file_name + "/classification", json={"modality": modality1, "replace": file_cls})
    assert r.ok
    ...
    r = as_admin.post("/projects/" + project + "/files/" + file_name + "/classification", json={"modality": None, "replace": file_cls})
    assert r.status_code == 422
    ...
    r = as_admin.post("/projects/" + project + "/files/" + "madeup.txt" + "/classification", json={"add": {"Intent": ["Localizer"]}})
    assert r.status_code == 404

    # Attempt to delete from nonexistent file
    r = as_admin.post("/projects/" + project + "/files/" + "madeup.txt" + "/classification", json={"delete": {"Intent": ["Localizer"]}})
    assert r.status_code == 404

    # Attempt to replae nonexistent file
    r = as_admin.post("/projects/" + project + "/files/" + "madeup.txt" + "/classification", json={"replace": {"Intent": ["Localizer"]}})
    assert r.status_code == 404
...
def test_classification_change_triggers_job(randstr, data_builder, as_admin, api_db, file_form):
    ...
    # Add rule
    rule = {"gear_id": gear, "name": "classification-job-trigger-rule", "any": [], "not": [], "all": [{"type": "file.classification", "value": "Localizer"}]}
    ...
    # Add modality to file
    r = as_admin.put("/acquisitions/" + acquisition + "/files/" + file_name, json={"modality": "MR"})

    ## SETUP COMPLETE

    # Test adding classification that doesn't trigger rule
    r = as_admin.post("/acquisitions/" + acquisition + "/files/" + file_name + "/classification", json={"add": {"Intent": ["Functional"]}})
    assert r.ok
    assert r.json()["jobs_spawned"] == 0

    # Test adding classification that does trigger rule
    r = as_admin.post("/acquisitions/" + acquisition + "/files/" + file_name + "/classification", json={"add": {"Intent": ["Localizer"]}})
    assert r.ok
