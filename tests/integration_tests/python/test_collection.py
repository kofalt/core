def test_collections(data_builder, as_admin, as_user, as_public):
    ...
    r = as_public.post("/collections", json={"label": "SciTran/Testing"})
    assert r.status_code == 403

    # create and delete collection as user
    r = as_user.post("/collections", json={"label": "SciTran/Testing"})
    assert r.ok
    ...
    # create collection
    r = as_admin.post("/collections", json={"label": "SciTran/Testing"})
    assert r.ok
    collection = r.json()["_id"]
    ...
    r = as_admin.put("/collections/" + collection, json={"contents": {"operation": "add", "nodes": [{"level": "session", "_id": session}]}})
    assert r.ok
    ...
    r = as_user.put("/collections/" + collection, json={"contents": {"operation": "add", "nodes": [{"level": "session", "_id": session2}]}})
    assert r.ok
...
def test_collection_stats(data_builder, as_admin, as_user, as_public):
    ...
    # === Empty Collection
    # create collection
    r = as_admin.post("/collections", json={"label": "SciTran/TestingStats"})
    assert r.ok
    collection = r.json()["_id"]
    ...
    # add session to collection
    r = as_admin.put("/collections/" + collection, json={"contents": {"operation": "add", "nodes": [{"level": "session", "_id": session_1}, {"level": "session", "_id": session_2}]}})
    assert r.ok
