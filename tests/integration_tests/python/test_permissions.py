def test_permissions(data_builder, as_admin):
    ...
    r = as_admin.post(permissions_path, json={"_id": "fake_user@yahoo.com", "access": "admin"})
    assert r.status_code == 402

    # Add permissions for user 1
    r = as_admin.post(permissions_path, json={"_id": user_1, "access": "ro"})
    assert r.ok
    ...
    r = as_admin.post(permissions_path, json={"_id": user_2, "access": "ro"})
    assert r.ok
