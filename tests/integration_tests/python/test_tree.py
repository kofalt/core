...
def test_fetch_tree_k(data_builder, as_admin):
    ...
    # Empty retrieval
    r = as_admin.post("/tree", json={"subjects": {"fields": ["code"]}}, params={"filter": "code=NOTEXIST"})
    assert r.ok
    assert len(r.json()) == 0

    # Simple retrieval
    r = as_admin.post("/tree", json={"subjects": {"fields": ["code"]}}, params={"filter": filter_str})
    assert r.ok
    ...
    r = as_admin.post("/tree", json={"subjects": {"fields": ["code"], "sessions": {"fields": ["label", "timestamp"], "sort": "_id:asc"}}}, params={"filter": filter_str})
    assert r.ok
    ...
    r = as_admin.post("/tree", json={"sessions": {"fields": ["label"], "acquisitions": {"fields": [], "sort": "_id:desc"}}}, params={"filter": filter_str, "sort": "_id:asc"})
    assert r.ok
    ...
    # 3 levels
    r = as_admin.post("/tree", json={"subjects": {"fields": ["code"], "sessions": {"fields": ["label", "timestamp"], "sort": "_id:asc", "acquisitions": {"fields": [], "sort": "_id:desc"}}}}, params={"filter": filter_str})
    assert r.ok
...
def test_fetch_tree_permissions(data_builder, as_admin, as_user, as_public):
    ...
    # Simple retrieval
    r = as_public.post("/tree", json={"subjects": {"fields": ["code"]}}, params={"filter": filter_str})
    assert r.status_code == 403

    # Masquerated as root
    r = as_user.post("/tree?root=true", json={"subjects": {"fields": ["code"]}}, params={"filter": filter_str})
    assert r.status_code == 403

    r = as_user.post("/tree?exhaustive=true", json={"subjects": {"fields": ["code"]}}, params={"filter": filter_str})
    assert r.status_code == 403

    # Retrieve as non-permissioned user
    r = as_user.post("/tree", json={"subjects": {"fields": ["code"]}}, params={"filter": filter_str})
    assert r.ok
    assert not r.json()
    ...
    # Exhaustive lists
    r = as_admin.post("/tree?root=true", json={"subjects": {"fields": ["code"]}}, params={"filter": filter_str})
    assert r.ok
    assert subjects == r.json()

    r = as_admin.post("/tree?exhaustive=true", json={"subjects": {"fields": ["code"]}}, params={"filter": filter_str})
    assert r.ok
    assert subjects == r.json()
...
def test_fetch_tree_filter_limit(data_builder, file_form, as_admin, as_root, api_db, legacy_cas_file):
    ...
    r = as_admin.post("/tree", json={"sessions": {"fields": ["label"], "acquisitions": {"fields": ["label"], "filter": "label=~acq2$"}}}, params={"filter": filter_str})  # Regex filter, matching *acq2
    assert r.ok
    sessions = r.json()
    ...
    r = as_admin.post("/tree", json={"sessions": {"fields": ["label"], "acquisitions": {"fields": ["label"], "limit": 1, "sort": "label:desc"}}}, params={"filter": filter_str})
    assert r.ok
    sessions = r.json()
...
def test_fetch_tree_parent(data_builder, as_admin):
    ...
    # Simple retrieval
    r = as_admin.post("/tree", json={"subjects": {"fields": ["code"], "project": {"fields": ["label"]}}}, params={"filter": filter_str})
    assert r.ok
    ...
    r = as_admin.post("/tree", json={"sessions": {"fields": ["label"], "group": {"fields": ["label"]}, "project": {"fields": ["label"]}, "subject": {"fields": ["code"]}}}, params={"sort": "_id:asc", "filter": filter_str})
    assert r.ok
    sessions = r.json()
...
def test_fetch_tree_files(data_builder, file_form, as_admin, as_root, api_db, legacy_cas_file):
    ...
    as_admin.post("/acquisitions/" + acquisition + "/files", files=file_form("test1.csv", meta={"name": "test1.csv", "type": "csv"}))

    as_admin.post("/acquisitions/" + acquisition + "/files", files=file_form("test2.csv", meta={"name": "test2.csv", "type": "csv"}))

    as_admin.post("/acquisitions/" + acquisition + "/files", files=file_form("test1.txt", meta={"name": "test1.txt", "type": "text"}))

    as_admin.post("/acquisitions/" + acquisition + "/files", files=file_form("test2.txt", meta={"name": "test2.txt", "type": "text"}))

    # also a deleted file to make sure it doesn't show up
    as_admin.post("/acquisitions/" + acquisition + "/files", files=file_form("deleted_test.csv", meta={"name": "deleted_test.csv", "type": "csv"}))
    r = as_admin.delete("/acquisitions/" + acquisition + "/files/deleted_test.csv")
    assert r.ok

    # All files
    # Permit sort & limit, not filtering
    r = as_admin.post("/tree", json={"acquisitions": {"fields": ["label"], "files": {"fields": ["name", "size", "type"]}}}, params={"filter": filter_str})
    assert r.ok
    ...
    # Test sort & limit
    r = as_admin.post("/tree", json={"acquisitions": {"fields": ["label"], "files": {"sort": "name:asc", "limit": 2, "join-origin": True, "fields": ["name", "size", "type"]}}}, params={"filter": filter_str})
    assert r.ok
    ...
    assert join_users["admin@user.com"] == {"_id": "admin@user.com", "firstname": "Test", "lastname": "Admin"}

    # Test sort descending
    r = as_admin.post("/tree", json={"acquisitions": {"fields": ["label"], "files": {"sort": "name:desc", "fields": ["name", "size", "type"]}}}, params={"filter": filter_str})
    assert r.ok
    ...
    r = as_admin.post("/tree", json={"acquisitions": {"fields": ["label"], "files": {"sort": "name:asc,modified:desc", "fields": ["name", "size", "type"]}}}, params={"filter": filter_str})
    assert r.status_code == 400

    # Does not support filtering
    r = as_admin.post("/tree", json={"acquisitions": {"fields": ["label"], "files": {"filter": "name=~csv$", "fields": ["name", "size", "type"]}}}, params={"filter": filter_str})
    assert r.status_code == 400
...
def test_fetch_tree_analyses(data_builder, as_admin, file_form):
    ...
    as_admin.post("/acquisitions/" + acquisition + "/files", files=file_form("input.csv", meta={"name": "input.csv", "type": "csv"}))

    as_admin.post("/acquisitions/" + acquisition + "/files", files=file_form("input.txt", meta={"name": "input.txt", "type": "txt"}))

    for container_type, container_id in [("projects", project), ("subjects", subject), ("sessions", session), ("acquisitions", acquisition)]:
        # Create ad-hoc analysis for container
        url = "/{}/{}/analyses".format(container_type, container_id)
        r = as_admin.post(url, json={"label": "analysis_label", "inputs": [{"type": "acquisition", "id": acquisition, "name": "input.csv"}, {"type": "acquisition", "id": acquisition, "name": "input.txt"}]})
        assert r.ok
        analysis_id = r.json()["_id"]

        # Manual upload
        r = as_admin.post("/analyses/" + analysis_id + "/files", files=file_form("output1.csv", "output2.csv", meta=[{"name": "output1.csv", "info": {"foo": "foo"}}, {"name": "output2.csv", "info": {"bar": "bar"}}]))
        assert r.ok

        # Then retrieve
        r = as_admin.post("/tree", json={container_type: {"fields": [], "analyses": {"fields": ["label"], "inputs": {"fields": ["name", "size"], "sort": "name:desc"}, "files": {"fields": ["name", "size"], "limit": 1, "sort": "name:asc"}}}}, params={"filter": filter_str})
        assert r.ok
    ...
    r = as_admin.post("/tree", json={"analyses": {"fields": ["label"], "project": {"fields": ["label"]}, "session": {"fields": ["label"]}, "acquisition": {"fields": ["label"]}}}, params={"filter": "parents.session={}".format(session), "sort": "_id:asc"})
    assert r.ok
...
def test_fetch_tree_jobs(data_builder, default_payload, as_admin, file_form):
    ...
    as_admin.post("/acquisitions/" + acquisition + "/files", files=file_form("test.zip", meta={"name": "test.zip", "type": "zip"}))

    # Add a job to the acquisition
    job_data = {"gear_id": gear, "inputs": {"dicom": {"type": "acquisition", "id": acquisition, "name": "test.zip"}}, "config": {"two-digit multiple of ten": 20}, "destination": {"type": "acquisition", "id": acquisition}, "tags": ["test-tag"]}

    r = as_admin.post("/jobs/add", json=job_data)
    assert r.ok
    job_id = r.json()["_id"]

    r = as_admin.post("/tree", json={"projects": {"fields": ["label"], "jobs": {"fields": ["gear_id", "gear_info", "created", "state"]}}}, params={"filter": filter_str})
    assert r.ok
    ...
    r = as_admin.post(
        "/tree", json={"jobs": {"fields": [], "group": {"fields": ["label"]}, "project": {"fields": ["label"]}, "subject": {"fields": ["code"]}, "session": {"fields": ["label"]}, "acquisition": {"fields": ["label"]}, "analysis": {"fields": ["label"]}}}, params={"filter": 'gear_id="{}"'.format(gear)}
    )
    assert r.ok
    jobs = r.json()
...
def test_fetch_tree_page_limits(data_builder, as_admin, file_form, api_db):
    ...
    api_db.subjects.insert_many([{"code": "s{:04}".format(i), "label": "s{:04}".format(i), "project": project, "parents": {"group": group, "project": project}, "created": now, "modified": now} for i in range(60)])
    ...
    try:
        ...
        r = as_admin.post("/tree?root=true", json={"subjects": {"fields": ["code"]}}, params={"filter": filter_str, "limit": 100})
        assert r.ok
        assert len(r.json()) == 50

        r = as_admin.post("/tree?root=true", json={"subjects": {"fields": ["code"]}}, params={"filter": filter_str, "limit": 10})
        assert r.ok
