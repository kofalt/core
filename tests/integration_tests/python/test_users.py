def test_users(data_builder, as_admin, as_user, as_public):
    ...
    r = as_admin.post("/users", json={"_id": "jane.doe@gmail.com", "lastname": "Doe", "email": "jane.doe@gmail.com"})
    assert r.status_code == 400
    assert "'firstname' is a required property" in r.text

    # Add new user
    new_user_id = "new@user.com"
    r = as_admin.post("/users", json={"_id": new_user_id, "firstname": "New", "lastname": "User"})
    assert r.ok
    ...
    r = as_admin.post("/users", json={"_id": new_user_id_admin, "firstname": "New2", "lastname": "User2"})
    assert r.ok
    r = as_admin.get("/users/" + new_user_id)
    assert r.ok
    ...
    r = as_admin.post("/projects/" + project + "/permissions", json={"_id": new_user_id_admin, "access": "ro"})
    assert r.ok
...
def test_user_info(as_user, as_admin):
    user_info = {"a": "b", "test": 123, "map": {"a": "b"}, "list": [1, 2, 3]}

    r = as_user.post("/users/self/info", json={"replace": user_info})
    assert r.ok

    r = as_admin.post("/users/self/info", json={"replace": user_info})
    assert r.ok
