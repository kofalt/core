...
def test_gear_invocation_and_suggest(data_builder, file_form, as_admin, as_user):
    ...
    assert as_admin.put("/collections/" + collection, json={"contents": {"operation": "add", "nodes": [{"level": "acquisition", "_id": acquisition3}]}}).ok
    ...
    as_user.post("/collections/" + collection + "/files", files=file_form("one.csv", meta={"name": "one.csv"}))
    as_user.post("/projects/" + project + "/files", files=file_form("one.csv", meta={"name": "one.csv"}))
    as_user.post("/sessions/" + session + "/files", files=file_form("one.csv", meta={"name": "one.csv"}))
    as_user.post("/sessions/" + session2 + "/files", files=file_form("one.csv", meta={"name": "one.csv"}))
    as_user.post("/acquisitions/" + acquisition + "/files", files=file_form("one.csv", meta={"name": "one.csv"}))
    as_user.post("/acquisitions/" + acquisition2 + "/files", files=file_form("one.csv", meta={"name": "one.csv"}))
    as_user.post("/acquisitions/" + acquisition3 + "/files", files=file_form("one.csv", meta={"name": "one.csv"}))

    # Add analysis
    analysis = as_user.post("/sessions/" + session + "/analyses", files=file_form("one.csv", meta={"label": "test", "outputs": [{"name": "one.csv"}]})).json()["_id"]
    analysis2 = as_user.post("/sessions/" + session2 + "/analyses", files=file_form("one.csv", meta={"label": "test", "outputs": [{"name": "one.csv"}]})).json()["_id"]
...
def test_gear_context(data_builder, default_payload, as_admin, as_user, randstr):
    ...
    project_info = {"test_context_value": 3, "context": {"test_context_value": "project_context_value"}}
    ...
    acquisition_info = {"context": {"test_context_value2": "acquisition_context_value"}}
    ...
    gear_doc["gear"]["inputs"] = {"test_context_value": {"base": "context"}, "test_context_value2": {"base": "context"}, "test_context_value3": {"base": "context"}, "text-file": {"base": "file", "type": {"enum": ["text"]}}}
    ...
    assert result["test_context_value"] == {"found": True, "value": "project_context_value", "container_type": "project", "id": project, "label": project_label}
    ...
    r = as_admin.post("/sessions/" + session + "/info", json={"set": {"context": {"test_context_value": "session_context_value", "test_context_value3": "session_context_value3"}}})
    assert r.ok
    ...
    assert result["test_context_value"] == {"found": True, "value": "session_context_value", "container_type": "session", "id": session, "label": session_label}

    assert result["test_context_value2"] == {"found": True, "value": "acquisition_context_value", "container_type": "acquisition", "id": acquisition, "label": acquisition_label}

    assert result["test_context_value3"] == {"found": True, "value": "session_context_value3", "container_type": "session", "id": session, "label": session_label}
...
def test_filter_gears_read_only_key(randstr, data_builder, default_payload, as_admin):
    ...
    gear_doc = default_payload["gear"]
    gear_doc["gear"]["name"] = gear_name
    gear_doc["gear"]["inputs"] = {"api_key": {"base": "api-key", "read-only": True}}
