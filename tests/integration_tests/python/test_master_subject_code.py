...
def test_master_subject_code(as_public, as_admin, api_db):
    ...
    resp = as_admin.post("/subjects/master-code", json={"use_patient_id": False, "first_name": "X", "last_name": "Y"})
    assert resp.status_code == 400

    # invalid input payload, invalid date format
    resp = as_admin.post("/subjects/master-code", json={"use_patient_id": False, "first_name": "X", "last_name": "Y", "date_of_birth": "1970.01.01"})
    assert resp.status_code == 400

    resp = as_admin.post("/subjects/master-code", json={"use_patient_id": False, "first_name": " \n", "last_name": "\t\t", "date_of_birth": "1980-02-02"})
    assert resp.status_code == 400

    resp = as_admin.post("/subjects/master-code", json={"use_patient_id": False, "first_name": "   JOHN   ", "last_name": "  SMith   ", "date_of_birth": "1980-02-02"})
    assert resp.status_code == 200
    msc = api_db.master_subject_codes.find_one({"_id": resp.json()["code"]})
    ...
    resp = as_admin.post("/subjects/master-code", json={"use_patient_id": True, "patient_id": "XYZ", "first_name": "First", "last_name": "Last", "date_of_birth": "1999-02-02"})
    assert resp.status_code == 200
    assert subj_code_2 == resp.json()["code"]

    # now we can get the previous subject code via first name, last name, date of birth
    resp = as_admin.post("/subjects/master-code", json={"use_patient_id": False, "first_name": "First", "last_name": "Last", "date_of_birth": "1999-02-02"})
    assert resp.status_code == 200
    assert subj_code_2 == resp.json()["code"]

    # use first name, last name, date of birth
    resp = as_admin.post("/subjects/master-code", json={"use_patient_id": False, "first_name": "X", "last_name": "Y", "date_of_birth": "1990-01-01"})
    assert resp.status_code == 200
    ...
    resp = as_admin.post("/subjects/master-code", json={"use_patient_id": True, "patient_id": "123456", "first_name": "X", "last_name": "Y", "date_of_birth": "1990-01-01"})
    assert resp.status_code == 200
    assert subj_code_3 != resp.json()["code"]
    ...
    resp = as_admin.post("/subjects/master-code", json={"use_patient_id": True, "patient_id": "123456"})
    assert resp.status_code == 200
...
def test_master_subject_code_indexes(api_db):
    ...
    api_db.master_subject_codes.insert_one({"_id": "CCC555", "first_name": "A", "last_name": "B", "date_of_birth": "1970-01-01"})
    ...
    with pytest.raises(errors.DuplicateKeyError):
        api_db.master_subject_codes.insert_one({"_id": "FFF555", "first_name": "A", "last_name": "B", "date_of_birth": "1970-01-01"})
    ...
    api_db.master_subject_codes.insert_one({"_id": "DDD555", "first_name": "C", "last_name": "D", "date_of_birth": "1970-01-01"})

    # insert the first patient with patient_id
    api_db.master_subject_codes.insert_one({"_id": "FFF555", "first_name": "A", "last_name": "B", "date_of_birth": "1970-01-01", "patient_id": "MRN-11111"})

    # patient id is unique
    with pytest.raises(errors.DuplicateKeyError):
        api_db.master_subject_codes.insert_one({"_id": "GGG555", "first_name": "A", "last_name": "B", "date_of_birth": "1970-01-01", "patient_id": "MRN-11111"})

    # patient id is part of the compound index but it also has a partial unique index
    with pytest.raises(errors.DuplicateKeyError):
        api_db.master_subject_codes.insert_one({"_id": "GGG555", "first_name": "E", "last_name": "F", "date_of_birth": "1970-01-01", "patient_id": "MRN-11111"})
    ...
    api_db.master_subject_codes.insert_one({"_id": "GGG555", "first_name": "A", "last_name": "B", "date_of_birth": "1970-01-01", "patient_id": "MRN-22222"})
...
def test_master_subject_code_verify_endpoint(as_public, as_user, as_admin):
    ...
    resp = as_admin.post("/subjects/master-code", json={"use_patient_id": True, "patient_id": "MRN-12345"})
