...
subject1 = {"code": "1001", "sex": "male", "age": years_to_secs(27), "cohort": "Control", "type": "human"}

subject2 = {"code": "1002", "sex": "female", "age": years_to_secs(33), "cohort": "Study", "type": "human"}
...
def json_test_data(name, rows=5):
    ...
    for i in range(row_count):
        output.append(collections.OrderedDict([("name", name), ("aValue", i), ("value2", i * 2)]))
...
def test_adhoc_data_view_permissions(data_builder, as_admin, as_user):
    project = data_builder.create_project(label="test-project")
    r = as_user.post("/views/data?containerId={}".format(project), json={"columns": [{"src": "subject.code", "dst": "subject"}]})
...
def test_adhoc_data_view_column_validation(data_builder, file_form, as_admin):
    ...
    r = as_admin.post("/views/data?containerId={}".format(project), json={"columns": [{"src": "session.permissions"}]})
    assert r.status_code == 400

    r = as_admin.post("/views/data?containerId={}".format(project), json={"columns": [{"src": "session.files"}]})
    assert r.status_code == 400
...
def test_adhoc_data_view_empty_result(data_builder, file_form, as_admin):
    # JSON
    project = data_builder.create_project(label="test-project")
    r = as_admin.post("/views/data?containerId={}".format(project), json={"includeIds": False, "includeLabels": False, "columns": [{"src": "project.label", "dst": "project"}, {"src": "acquisition.label", "dst": "acquisition"}]})
    assert r.ok
    ...
    # JSON (flat)
    project = data_builder.create_project(label="test-project")
    r = as_admin.post("/views/data?containerId={}&format=json-flat".format(project), json={"includeIds": False, "includeLabels": False, "columns": [{"src": "project.label", "dst": "project"}, {"src": "acquisition.label", "dst": "acquisition"}]})
    assert r.ok
    rows = r.json()
    assert len(rows) == 0

    # JSON row-column
    r = as_admin.post("/views/data?containerId={}&format=json-row-column".format(project), json={"includeIds": False, "includeLabels": False, "columns": [{"src": "project.label", "dst": "project"}, {"src": "acquisition.label", "dst": "acquisition"}]})
    assert r.ok
    data = r.json()["data"]
...
def test_adhoc_data_view_metadata_only(data_builder, file_form, as_admin):
    ...
    r = as_admin.post(
        "/views/data?containerId={}".format(project),
        json={
            "includeIds": True,
            "includeLabels": False,
            "columns": [
...
def test_adhoc_data_view_flatten_info(data_builder, file_form, as_admin):
    ...
    as_admin.post("/sessions/" + session1 + "/info", json={"replace": {"value": 4, "nested": {"value": 12, "value2": 8}}})
    as_admin.post("/sessions/" + session2 + "/info", json={"replace": {"value": 6, "value2": 19, "nested": {"value": 23}}})

    # Keep src key
    r = as_admin.post("/views/data?containerId={}&format=csv".format(project), json={"includeIds": False, "includeLabels": False, "columns": [{"src": "session.id"}, {"src": "session.info"}]})
    ...
    # Remap to dst key
    r = as_admin.post("/views/data?containerId={}&format=csv".format(project), json={"includeIds": False, "includeLabels": False, "columns": [{"src": "session.id"}, {"src": "session.info", "dst": "session_info"}]})
...
def test_adhoc_data_view_session_target(data_builder, file_form, as_admin):
    ...
    r = as_admin.post(
        "/views/data?containerId={}&format=json-flat".format(session2),
        json={
            "includeIds": False,
            "includeLabels": False,
            "columns": [{"src": "project"}, {"src": "subject.code", "dst": "subject"}, {"src": "subject.sex"}, {"src": "subject.type"}, {"src": "subject.cohort"}, {"src": "session.label", "dst": "session"}, {"src": "session.age_years"}, {"src": "acquisition.label", "dst": "acquisition"}],
        },
    )
...
def test_adhoc_data_view_csv_files(data_builder, file_form, as_admin):
    ...
    r = as_admin.post(
        ...
        json={
            "includeIds": False,
            "includeLabels": False,
            "columns": [{"src": "subject.label"}, {"src": "subject.age"}, {"src": "subject.sex"}],
            "fileSpec": {"container": "acquisition", "filter": {"value": "*.csv"}, "columns": [{"src": "name"}, {"src": "value", "type": "int"}, {"src": "value2", "type": "float"}]},
        },
    )
...
def test_adhoc_data_view_json_row_column_format(data_builder, file_form, as_admin):
    ...
    r = as_admin.post(
        "/views/data?containerId={}&format=json-row-column".format(project),
        json={"includeIds": False, "includeLabels": False, "columns": [{"src": "subject.code", "dst": "subject"}, {"src": "subject.age"}, {"src": "subject.sex"}], "fileSpec": {"container": "acquisition", "filter": {"value": "*.csv"}}},
    )
...
def test_adhoc_data_view_csv_format(data_builder, file_form, as_admin):
    ...
    r = as_admin.post("/views/data?containerId={}&format=csv".format(project), json={"includeIds": False, "includeLabels": False, "fileSpec": {"container": "acquisition", "filter": {"value": "*.csv"}}})
...
def test_adhoc_data_view_tsv_format(data_builder, file_form, as_admin):
    ...
    r = as_admin.post(
        "/views/data?containerId={}&format=tsv".format(project), json={"includeIds": False, "includeLabels": False, "columns": [{"src": "subject.code", "dst": "subject"}, {"src": "subject.age"}, {"src": "subject.sex"}], "fileSpec": {"container": "acquisition", "filter": {"value": "*.csv"}}}
    )
...
def test_adhoc_data_view_tsv_file(data_builder, file_form, as_admin):
    ...
    r = as_admin.post("/views/data?containerId={}".format(project), json={"includeIds": False, "includeLabels": False, "columns": [{"src": "subject.code", "dst": "subject"}], "fileSpec": {"container": "acquisition", "filter": {"value": "*.tsv"}}})
...
def test_adhoc_data_view_json_list_file(data_builder, file_form, as_admin):
    ...
    r = as_admin.post("/views/data?containerId={}".format(project), json={"includeIds": False, "includeLabels": False, "columns": [{"src": "subject.code", "dst": "subject"}], "fileSpec": {"container": "acquisition", "filter": {"value": "*.json"}}})
...
def test_adhoc_data_view_missing_data_csv_files(data_builder, file_form, as_admin):
    ...
    # ============================
    r = as_admin.post("/views/data?containerId={}".format(project), json={"includeIds": False, "includeLabels": False, "columns": [{"src": "subject.code", "dst": "subject"}, {"src": "subject.age"}, {"src": "subject.sex"}], "fileSpec": {"container": "acquisition", "filter": {"value": "*.csv.gz"}}})
    ...
    r = as_admin.post(
        "/views/data?containerId={}".format(project),
        json={"includeIds": False, "includeLabels": False, "missingDataStrategy": "drop-row", "columns": [{"src": "subject.code", "dst": "subject"}, {"src": "subject.age"}, {"src": "subject.sex"}], "fileSpec": {"container": "acquisition", "filter": {"value": "*.csv.gz"}}},
    )
...
def test_adhoc_data_view_zip_members(data_builder, file_form, as_admin):
    ...
    r = as_admin.post(
        "/views/data?containerId={}".format(project),
        json={"includeIds": False, "includeLabels": False, "columns": [{"src": "subject.code", "dst": "subject"}, {"src": "subject.age"}, {"src": "subject.sex"}], "fileSpec": {"container": "acquisition", "filter": {"value": "*.zip"}, "zipMember": {"value": "dir2/*.csv"}}},
    )
...
def test_adhoc_data_view_analyses_files(data_builder, file_form, as_admin, as_drone, api_db):
    ...
    r = as_admin.post("/sessions/" + session + "/analyses", json={"label": "analysis-1", "job": {"gear_id": gear1, "inputs": {"csv": {"type": "acquisition", "id": acquisition, "name": "input.csv"}}}})
    assert r.ok
    ...
    # Upload output file
    file_form1 = file_form(("values.csv", csv_test_data("a1")))
    r = as_drone.post("/engine", params={"level": "analysis", "id": analysis1, "job": job1}, files=file_form1)
    assert r.ok

    # Create job-based analysis 2
    r = as_admin.post("/sessions/" + session + "/analyses", json={"label": "second-analysis", "job": {"gear_id": gear2, "inputs": {"csv": {"type": "acquisition", "id": acquisition, "name": "input.csv"}}}})
    assert r.ok
    ...
    r = as_drone.post("/engine", params={"level": "analysis", "id": analysis2, "job": job2}, files=[("file", file_form2["file"]), ("file", file_form3["file"])])
    assert r.ok

    # Execute data view, match on label
    r = as_admin.post(
        "/views/data?containerId={}".format(project),
        json={"includeIds": False, "includeLabels": False, "columns": [{"src": "subject.code", "dst": "subject"}, {"src": "subject.age"}, {"src": "subject.sex"}], "fileSpec": {"container": "session", "filter": {"value": "*.csv"}, "analysisFilter": {"label": {"value": "analysis*"}}}},
    )
    ...
    r = as_admin.post(
        ...
        json={
            "includeIds": False,
            "includeLabels": False,
            "columns": [{"src": "subject.code", "dst": "subject"}, {"src": "subject.age"}, {"src": "subject.sex"}, {"src": "analysis.label", "dst": "analysis"}, {"src": "file.name", "dst": "filename"}, {"src": "file.row_number", "dst": "row"}],
            "fileSpec": {"container": "session", "filter": {"value": "*.csv"}, "match": "all", "analysisFilter": {"label": {"value": "second-analysis"}}},
        },
    )
    ...
    r = as_admin.post(
        ...
        json={
            "includeIds": False,
            "includeLabels": False,
            "columns": [{"src": "subject.code", "dst": "subject"}, {"src": "subject.age"}, {"src": "subject.sex"}, {"src": "analysis.label", "dst": "analysis"}, {"src": "file.name", "dst": "filename"}],
            "fileSpec": {"container": "session", "filter": {"value": "*.csv"}, "match": "all", "analysisFilter": {"label": {"value": "second-analysis"}}, "processFiles": False},
        },
    )
    ...
    r = as_admin.post(
        "/views/data?containerId={}".format(project),
        json={"includeIds": False, "includeLabels": False, "columns": [{"src": "file.name", "dst": "filename"}], "fileSpec": {"container": "session", "filter": {"value": "*.csv"}, "match": "all", "analysisFilter": {"gear.name": {"value": "data-view-gear1"}}, "processFiles": False}},
    )
    ...
    r = as_admin.post(
        ...
        json={
            "includeIds": False,
            "includeLabels": False,
            "columns": [{"src": "file.name", "dst": "filename"}],
            "fileSpec": {"container": "session", "filter": {"value": "*.csv"}, "match": "all", "analysisFilter": {"gear.name": {"value": "data-view-gear?"}, "gear.version": {"value": "0.0.13"}}, "processFiles": False},
        },
    )
    ...
    r = as_admin.post("/views/data?containerId={}".format(project), json={"includeIds": False, "includeLabels": False, "columns": [{"src": "file.name"}], "fileSpec": {"container": "session", "filter": {"value": "*.csv"}, "analysisFilter": {"label": {"value": "invalid-match"}}}})
...
def test_site_data_view(as_admin, as_user):
    view = {"label": "test-site-view", "columns": [{"src": "acquisition.label"}]}
...
def test_group_data_view(as_admin, as_user, data_builder):
    ...
    view = {"label": "test-group-view", "columns": [{"src": "acquisition.label"}]}
...
def test_project_data_view(as_admin, as_user, as_public, data_builder, file_form):
    ...
    view = {"label": "test-project-view", "includeIds": False, "includeLabels": False, "columns": [{"src": "subject.label"}, {"src": "subject.age"}, {"src": "subject.sex"}], "fileSpec": {"container": "acquisition", "filter": {"value": "*.csv"}}}
...
def test_data_view_filtering(data_builder, file_form, as_admin):
    ...
    r = as_admin.post("/views/data?containerId={}&sort=subject.label:asc".format(project), json={"columns": [{"src": "session.label"}]})
    assert r.status_code == 422

    # Validate that we can't filter on unselected containers
    r = as_admin.post("/views/data?containerId={}&filter=acquisition.label=foobar".format(project), json={"columns": [{"src": "session.label"}]})
    assert r.status_code == 400

    r = as_admin.post("/views/data?containerId={}&filter=subject.code=1001".format(project), json={"includeIds": True, "includeLabels": True, "columns": [{"src": "session.label"}]})

    assert r.ok
    rows = r.json()["data"]
    assert len(rows) == 1

    r = as_admin.post("/views/data?containerId={}&filter=subject.cohort=Control".format(project), json={"includeIds": True, "includeLabels": True, "columns": [{"src": "session.label"}]})
    ...
    r = as_admin.post("/views/data?containerId={}&filter=subject.code=~%5Cd%2B".format(project), json={"includeIds": True, "includeLabels": True, "columns": [{"src": "session.label"}]})
    ...
    r = as_admin.post("/views/data?containerId={}&filter=session.tags=tag1".format(project), json={"includeIds": True, "includeLabels": True, "columns": [{"src": "session.label"}]})
...
def test_data_view_skip_and_limit(data_builder, file_form, as_admin):
    ...
    # Test column aliases as well
    r = as_admin.post(
        "/views/data?containerId={}&skip=2&limit=2".format(project),
        json={
            "includeIds": False,
            "includeLabels": False,
            "columns": [{"src": "subject.label"}, {"src": "subject.age"}, {"src": "subject.sex"}],
...
def test_save_data_view_to_container(data_builder, file_form, as_admin, as_user, as_public):
    ...
    view = {"includeIds": True, "includeLabels": False, "columns": [{"src": "project.label"}, {"src": "subject.label"}, {"src": "subject.age"}, {"src": "subject.sex"}, {"src": "session.label"}, {"src": "acquisition.label"}]}

    # No public access
    r = as_public.post("/views/save?containerId={}".format(project), json={"view": view, "containerType": "project", "containerId": project, "filename": "data_view.json"})
    assert r.status_code == 403
    ...
    r = as_user.post("/views/save?containerId={}".format(project), json={"view": view, "containerType": "project", "containerId": project, "filename": "data_view.json"})
    assert r.status_code == 403

    # Execute adhoc view and save it to project
    r = as_admin.post("/views/save?containerId={}".format(project), json={"view": view, "containerType": "project", "containerId": project, "filename": "data_view.json"})
    ...
    r = as_admin.post("/views/save?containerId={}".format(project), json={"viewId": view_id, "containerType": "session", "containerId": session, "filename": "saved_data_view.json"})
    assert r.ok
...
def test_adhoc_data_view_deleted_container(data_builder, file_form, as_admin):
    ...
    assert as_admin.post("/acquisitions/" + acquisition1 + "/files", files=file_form(("test.txt"))).ok
    assert as_admin.post("/acquisitions/" + acquisition1 + "/files/test.txt/info", json={"replace": {"Key": "Value 1"}})

    assert as_admin.post("/acquisitions/" + acquisition2 + "/files", files=file_form(("test.txt"))).ok
    assert as_admin.post("/acquisitions/" + acquisition2 + "/files/test.txt/info", json={"replace": {"Key": "Value 2"}})

    # Control Test that files show up with two rows
    r = as_admin.post(
        "/views/data?containerId={}".format(project),
        json={"includeLabels": False, "columns": [{"src": "project.label", "dst": "project"}, {"src": "acquisition.label", "dst": "acquisition"}, {"src": "file.info.Key", "dst": "file"}], "fileSpec": {"container": "acquisition", "filter": {"value": "*.txt"}, "match": "all"}},
    )
    assert r.ok
    assert r.headers["content-disposition"] == 'attachment; filename="view-data.json"'
    rows = r.json()["data"]
