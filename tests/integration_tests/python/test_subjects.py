...
def test_subject_endpoints(data_builder, as_admin, as_public, file_form):
    ...
    # PUT /subjects/x
    r = as_admin.put("/subjects/" + subject, json={"sex": "female", "label": "test_2"})
    assert r.ok
...
def test_subject_jobs(api_db, data_builder, as_admin, as_drone, file_form):
    ...
    r = as_admin.post("/subjects/" + subject + "/analyses", json={"label": "online", "job": {"gear_id": gear, "inputs": {"csv": {"type": "subject", "id": subject, "name": "input.csv"}}}})
    assert r.ok
    analysis = r.json()["_id"]
    ...
    # Engine upload
    r = as_drone.post("/engine", params={"level": "analysis", "id": analysis, "job": job}, files=file_form("output.csv", meta={"type": "tabular data"}))
    assert r.ok
    ...
    r = as_admin.post("/jobs/add", json={"gear_id": gear, "inputs": {"csv": {"type": "subject", "id": subject, "name": "input.csv"}}, "destination": {"type": "subject", "id": subject}})
    assert r.ok
    job = r.json()["_id"]
    ...
    r = as_drone.post("/engine", params={"level": "subject", "id": subject, "job": job, "job_ticket": job_ticket}, files=file_form("result.txt", meta={"subject": {"files": [{"name": "result.txt", "type": "text"}]}}))
    assert r.ok
...
def test_subject_move_via_session(data_builder, as_admin, as_user, default_payload, file_form):
    ...
    # Create a job
    job_data = {"gear_id": gear, "inputs": {"dicom": {"type": "acquisition", "id": acquisition, "name": "test.zip"}}, "config": {"two-digit multiple of ten": 40}, "destination": {"type": "acquisition", "id": acquisition}, "tags": ["test-tag"]}
    ...
    assert project_2 == as_admin.get("/sessions/" + session_1).json()["subject"]["project"] == as_admin.get("/subjects/" + subject_1).json()["project"]
    ...
    assert project_1 == as_admin.get("/sessions/" + session_1).json()["subject"]["project"] == as_admin.get("/subjects/" + subject_2).json()["project"]
    assert subject_1 in [s["_id"] for s in as_admin.get("/projects/" + project_2 + "/subjects").json()]
...
def test_subject_fields(data_builder, as_admin):
    subject_fields = dict(code="test", label="test", cohort="Study", type="animal", species="dog", strain="free-string", public=True)
