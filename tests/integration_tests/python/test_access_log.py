...
def test_access_log_succeeds(data_builder, as_admin, log_db):
    ...
    r = as_admin.post("/login", json={"auth_type": "api-key", "code": api_key})
    assert r.ok
    ...
    # Upload files
    r = as_admin.post("/projects/" + project + "/files", files={"file": (file_name, "test-content")})
    assert r.ok
    ...
    # Upload another file
    r = as_admin.post("/sessions/" + session + "/files", files={"file": (file_name, "test-content")})
    assert r.ok
    ...
    log_records_count_before = log_db.access_log.count({})
    r = as_admin.post("/download", params={"bulk": True}, json={"files": [{"container_name": "project", "container_id": project, "filename": file_name}, {"container_name": "session", "container_id": session, "filename": file_name}]})
    assert r.ok
    ticket_id = r.json()["ticket"]
    file_count = r.json()["file_cnt"]
    r = as_admin.get("/download", params={"ticket": ticket_id})
    ...
    r = as_admin.post("/projects/" + project + "/files", files={"file": (file_name, "different-content")})
    assert r.ok
...
def test_bulk_access(data_builder, as_admin, log_db):
    ...
    entries = [(AccessType.view_container, {"group": {"id": "test", "label": "Test Group"}}), (AccessType.view_file, {"group": {"id": "test", "label": "Test Group"}, "project": {"id": project, "label": "Test Project"}, "file": {"name": "example.csv"}})]
...
def test_job_access(data_builder, as_admin, as_drone, log_db, default_payload, file_form, api_db):
    ...
    # Add info to file
    r = as_admin.post("/acquisitions/" + acquisition + "/files/test.zip/info", json={"replace": {"a": "b"}})
    assert r.ok
    ...
    r = as_admin.post("/acquisitions/" + acquisition + "/files/analysis-test.zip/info", json={"replace": {"a": "b"}})
    assert r.ok
    ...
    # upload metadata
    produced_metadata = {"session": {"label": "engine session", "subject": {"sex": "male", "age": 86400}, "info": {"test": "s"}}}

    # engine upload
    r = as_drone.post("/engine", params={"level": "acquisition", "id": acquisition, "job": job_id, "job_ticket": job_ticket["_id"]}, files=file_form(meta=produced_metadata))
    assert r.ok

    # Post complete
    r = as_drone.post("/jobs/" + job_id + "/complete", json={"success": True, "profile": {"elapsed_time_ms": 36501, "preparation_time_ms": 2515, "upload_time_ms": 1017}})
    assert r.ok
    ...
    # Verify that produced metadata and info don't appear on a list endpoint
    r = as_admin.get("/sessions/" + session + "/jobs", params={"filter": "_id={}".format(job_id)})
    assert r.ok
    ...
    r = as_admin.post("/projects/" + project + "/analyses", json={"label": "online", "job": {"gear_id": gear, "inputs": {"dicom": {"type": "acquisition", "id": acquisition, "name": "analysis-test.zip"}}}})
    assert r.ok
    analysis = r.json()["_id"]
    ...
    api_db.acquisitions.update_one({"_id": bson.ObjectId(acquisition)}, {"$pull": {"files": {"name": "test.zip"}}})
