...
def test_failed_job_output(data_builder, default_payload, as_user, as_admin, as_drone, api_db, file_form, with_site_settings):
    # create gear
    gear_doc = default_payload['gear']['gear']
    gear_doc['inputs'] = {
        'dicom': {
...
def test_job_detail(data_builder, default_payload, as_admin, as_user, as_drone, as_public, file_form, api_db, with_site_settings):
    # Dupe of test_queue.py
    gear_doc = default_payload['gear']['gear']
    gear_doc['inputs'] = {
        'zip': {
...
def test_job_providers(compute_provider, data_builder, default_payload, as_public, as_user, as_admin, api_db, file_form):
...
def test_jobs_subjects_user_filter(data_builder, as_user, as_admin, default_payload, file_form):
    group = data_builder.create_group(providers={})  # Create group without providers
    project = data_builder.create_project(group=group)
    subject = data_builder.create_subject(project=project, code='testcode1234')
    session = data_builder.create_session(project=project)
    acquisition = data_builder.create_acquisition(session=session)

    assert as_admin.post('/sessions/' + session + '/files', files=file_form('test.zip')).ok
    gear_doc = default_payload['gear']['gear']
    gear_doc['inputs'] = {
        'dicom': {
            'base': 'file'
    ...
    #job_data['compute_provider_id'] = r.json()['compute_provider']
    r = as_admin.post('/jobs/add', json=job_data)
    assert r.ok
    job_id = r.json()['_id']


    # User can see the job when permission on the project
    user_id = as_user.get('/users/self').json()['_id']
    assert as_admin.post('/projects/' + project + '/permissions', json={
        'access': 'admin',
        '_id': user_id
