...
def test_download_k(data_builder, file_form, as_admin, as_user, api_db, legacy_cas_file):
    ...
    file_name = "test.csv"
    as_user.post("/acquisitions/" + acquisition + "/files", files=file_form(file_name, meta={"name": file_name, "type": "csv"}))

    as_user.post("/acquisitions/" + acquisition2 + "/files", files=file_form(file_name, meta={"name": file_name, "type": "csv"}))

    as_user.post("/acquisitions/" + acquisition3 + "/files", files=file_form("test.txt", meta={"name": file_name, "type": "text"}))

    as_user.post("/acquisitions/" + acquisition4 + "/files", files=file_form("test.txt", meta={"name": file_name, "type": "text"}))

    as_user.post("/sessions/" + session + "/files", files=file_form(file_name, meta={"name": file_name, "type": "csv", "tags": ["plus"]}))

    as_user.post("/projects/" + project + "/files", files=file_form(file_name, meta={"name": file_name, "type": "csv", "tags": ["plus", "minus"]}))

    # also a deleted file to make sure it doesn't show up
    as_admin.post("/acquisitions/" + acquisition + "/files", files=file_form(file_name, meta={"name": "deleted_" + file_name, "type": "csv"}))
    ...
    r = as_admin.post("/download", json={"optional": False, "filters": [{"tags": {"-": ["minus"]}}], "nodes": [{"level": "project", "_id": project}]})
    assert r.ok
    ...
    r = as_user.post("/download", json={"optional": False, "filters": [{"tags": {"-": ["minus"]}}], "nodes": [{"level": "project", "_id": project}]})
    assert r.ok
    ticket = r.json()["ticket"]
    ...
    r = as_user.post("/acquisitions/" + acquisition3 + "/files", files=file_form(file_name, meta={"name": file_name, "type": "csv"}))
    assert r.ok

    r = as_user.post("/download", json={"optional": False, "nodes": [{"level": "acquisition", "_id": acquisition}, {"level": "acquisition", "_id": acquisition3}]})
    assert r.ok
    ticket = r.json()["ticket"]
    ...
    update_result = api_db.downloads.update_one({"_id": ticket}, {"$set": {"ip": "255.255.255.255"}})
    ...
    r = as_user.post("/download", json={"optional": False, "nodes": [{"level": "project", "_id": missing_object_id}, {"level": "session", "_id": missing_object_id}, {"level": "acquisition", "_id": missing_object_id}]})
    assert r.status_code == 404

    # Try to retrieve ticket for bulk download w/ invalid container name
    # (not project|session|acquisition)
    r = as_user.post("/download", params={"bulk": "true"}, json={"files": [{"container_name": "collection", "container_id": missing_object_id, "filename": "nosuch.csv"}]})
    assert r.status_code == 400

    # Try to retrieve ticket for bulk download referencing nonexistent file
    r = as_user.post("/download", params={"bulk": "true"}, json={"files": [{"container_name": "project", "container_id": project, "filename": "nosuch.csv"}]})
    assert r.status_code == 404

    # Retrieve ticket for bulk download
    r = as_user.post("/download", params={"bulk": "true"}, json={"files": [{"container_name": "project", "container_id": project, "filename": file_name}]})
    assert r.ok
    ticket = r.json()["ticket"]
    ...
    r = as_user.post("/download", json={"optional": False, "nodes": [{"level": "project", "_id": project_legacy}]})
    assert r.ok
    ticket = r.json()["ticket"]
    ...
    api_db.acquisitions.update_one({"_id": ObjectId(acquisition)}, {"$set": {"files.0._id": "not_available"}})
...
def test_filelist_range_download(data_builder, as_user, as_admin, file_form):
    ...
    r = as_user.get(session_files + "/one.csv", params={"ticket": ticket, "view": "true"}, headers={"Range": "bytes=0-"})
    assert r.ok
    assert r.content == "123456789"
    ...
    r = as_user.get(session_files + "/one.csv", params={"ticket": ticket, "view": "true"}, headers={"range": "bytes=0-0"})
    assert r.ok
    assert r.content == "1"
    ...
    r = as_user.get(session_files + "/one.csv", params={"ticket": ticket, "view": "true"}, headers={"Range": "bytes=0-1"})
    assert r.ok
    assert r.content == "12"
    ...
    r = as_user.get(session_files + "/one.csv", params={"ticket": ticket, "view": "true"}, headers={"Range": "lol=-5"})
    ...
    r = as_user.get(session_files + "/one.csv", params={"ticket": ticket, "view": "true"}, headers={"Range": "bytes=0-500"})
    assert r.ok
    assert r.content == "123456789"
    ...
    r = as_user.get(session_files + "/one.csv", params={"ticket": ticket, "view": "true"}, headers={"Range": "bytes=500-"})
    ...
    r = as_user.get(session_files + "/one.csv", params={"ticket": ticket, "view": "true"}, headers={"Range": "bytes=10-5"})
    assert r.ok
    assert r.content == "123456789"
    ...
    r = as_user.get(session_files + "/one.csv", params={"ticket": ticket, "view": "true"}, headers={"Range": "bytes-1+5"})
    assert r.ok
    assert r.content == "123456789"


@pytest.mark.skipif(not os.getenv("SCITRAN_PERSISTENT_FS_URL", "osfs").startswith("osfs"), reason="Only OSFS supports all of these special range formats.")
def test_filelist_advanced_range_download(data_builder, as_user, as_admin, file_form):
    ...
    r = as_user.get(session_files + "/one.csv", params={"ticket": ticket, "view": "true"}, headers={"Range": "bytes=-5"})
    assert r.ok
    ...
    r = as_user.get(session_files + "/one.csv", params={"ticket": ticket, "view": "true"}, headers={"Range": "bytes=-"})
    assert r.ok
    assert r.content == "123456789"
    ...
    r = as_user.get(session_files + "/one.csv", params={"ticket": ticket, "view": "true"}, headers={"Range": "bytes=r-0"})
    assert r.ok
    assert r.content == "123456789"
    ...
    # ASFS returns with status code 400
    r = as_user.get(session_files + "/one.csv", params={"ticket": ticket, "view": "true"}, headers={"Range": "bytes=0-bb"})
    assert r.ok
    assert r.content == "123456789"
    ...
    r = as_user.get(session_files + "/one.csv", params={"ticket": ticket, "view": "true"}, headers={"Range": "bytes=1-2, 3-4"})
    assert r.ok
...
def test_analysis_download(data_builder, file_form, as_user, as_admin, as_drone, default_payload):
    ...
    assert as_admin.post("/projects/" + project + "/permissions", json={"access": "admin", "_id": user_id}).ok

    # create (legacy) analysis for testing the download functionality
    r = as_user.post("/sessions/" + session + "/analyses", files=file_form("one.csv", ("two.zip", zip_cont), meta={"label": "test", "inputs": [{"name": "one.csv"}], "outputs": [{"name": "two.zip"}]}))
    assert r.ok
    analysis = r.json()["_id"]
...
def test_analyses_range_download(data_builder, as_user, as_admin, file_form):
    ...
    # create (legacy) analysis for testing the download functionality
    r = as_user.post("/sessions/" + session + "/analyses", files=file_form(("one.csv", "123456789"), ("two.zip", zip_cont), meta={"label": "test", "inputs": [{"name": "one.csv"}], "outputs": [{"name": "two.zip"}]}))
    assert r.ok
    analysis = r.json()["_id"]
    ...
    # download single file from byte 0 to end of file
    r = as_user.get(analysis_inputs + "/one.csv", params={"ticket": ticket, "view": "true"}, headers={"Range": "bytes=0-"})
    assert r.ok
    assert r.content == "123456789"

    # download single file's first byte by using lower case header
    r = as_user.get(analysis_inputs + "/one.csv", params={"ticket": ticket, "view": "true"}, headers={"range": "bytes=0-0"})
    assert r.ok
    assert r.content == "1"

    # download single file's first two byte
    r = as_user.get(analysis_inputs + "/one.csv", params={"ticket": ticket, "view": "true"}, headers={"Range": "bytes=0-1"})
    assert r.ok
    assert r.content == "12"

    # try to download single file with invalid unit
    r = as_user.get(analysis_inputs + "/one.csv", params={"ticket": ticket, "view": "true"}, headers={"Range": "lol=-5"})
    assert r.status_code == 200
    assert r.content == "123456789"

    # try to download single file with invalid range where the last byte is greater then the size of the file
    # in this case the whole file is returned
    r = as_user.get(analysis_inputs + "/one.csv", params={"ticket": ticket, "view": "true"}, headers={"Range": "bytes=0-500"})
    assert r.ok
    assert r.content == "123456789"

    # try to download single file with invalid range where the first byte is greater then the size of the file
    r = as_user.get(analysis_inputs + "/one.csv", params={"ticket": ticket, "view": "true"}, headers={"Range": "bytes=500-"})
    assert r.status_code == 416

    # try to download single file with invalid range first byte is greater than the last one
    r = as_user.get(analysis_inputs + "/one.csv", params={"ticket": ticket, "view": "true"}, headers={"Range": "bytes=10-5"})
    assert r.ok
    assert r.content == "123456789"

    # try to download single file with invalid range header syntax
    r = as_user.get(analysis_inputs + "/one.csv", params={"ticket": ticket, "view": "true"}, headers={"Range": "bytes-1+5"})
    assert r.ok
    assert r.content == "123456789"
...
def test_filters(data_builder, file_form, as_user, as_admin):
    ...
    as_user.post("/acquisitions/" + acquisition + "/files", files=file_form("test.csv", meta={"name": "test.csv", "type": "csv", "tags": ["red", "blue"]}))
    as_user.post("/acquisitions/" + acquisition + "/files", files=file_form("test.dicom", meta={"name": "test.dicom", "type": "dicom", "tags": ["red"]}))
    as_user.post("/acquisitions/" + acquisition2 + "/files", files=file_form("test.nifti", meta={"name": "test.nifti", "type": "nifti"}))
    r = as_user.get("/acquisitions/" + acquisition)
    assert r.ok

    # Malformed filters
    r = as_user.post("/download", json={"optional": False, "filters": [{"tags": "red"}], "nodes": [{"level": "session", "_id": session}]})
    assert r.status_code == 400

    # No filters
    r = as_user.post("/download", json={"optional": False, "nodes": [{"level": "session", "_id": session}]})
    assert r.ok
    ...
    # Use filter aliases
    r = as_user.post("/download", json={"optional": False, "filters": [{"tags": {"plus": ["red"]}}], "nodes": [{"level": "session", "_id": session}]})
    assert r.ok
    # 'plus' is same as using '+'
    assert r.json()["file_cnt"] == 2

    # Filter by type
    as_user.post("/acquisitions/" + acquisition + "/files", files=file_form("test", meta={"name": "test", "tags": ["red", "blue"]}))
    r = as_user.post("/download", json={"optional": False, "filters": [{"types": {"+": ["nifti"]}}], "nodes": [{"level": "session", "_id": session}]})
    assert r.ok
    assert r.json()["file_cnt"] == 1
    r = as_user.post("/download", json={"optional": False, "filters": [{"types": {"+": ["null"]}}], "nodes": [{"level": "session", "_id": session}]})
    assert r.ok
    assert r.json()["file_cnt"] == 1
...
def test_summary(data_builder, as_user, as_admin, file_form):
    ...
    r = as_user.post("/sessions/" + session + "/analyses", files=file_form(file_name, meta={"label": "test", "outputs": [{"name": file_name}]}))
    assert r.ok
    analysis = r.json()["_id"]
...
def test_full_project_download(data_builder, file_form, as_admin, as_root, as_drone, api_db):
    ...
    as_admin.post("/sessions/" + session + "/info", json={"replace": {"test": "test_data"}})

    # upload the same file to each container created and use different tags to
    # facilitate download filter tests:
    file_name = "test.csv"
    as_admin.post("/acquisitions/" + acquisition + "/files", files=file_form(file_name, meta={"name": file_name, "type": "csv", "tags": ["acq1"]}))

    as_admin.post("/acquisitions/" + acquisition2 + "/files", files=file_form(file_name, meta={"name": file_name, "type": "csv"}))

    as_admin.post("/acquisitions/" + acquisition3 + "/files", files=file_form("test.txt", meta={"name": file_name, "type": "text"}))

    as_admin.post("/acquisitions/" + acquisition4 + "/files", files=file_form("test.txt", meta={"name": file_name, "type": "text"}))

    as_admin.post("/sessions/" + session + "/files", files=file_form(file_name, meta={"name": file_name, "type": "csv", "tags": ["plus"]}))

    as_admin.post("/projects/" + project + "/files", files=file_form(file_name, meta={"name": file_name, "type": "csv", "tags": ["plus", "minus"]}))
    ...
    r = as_admin.post("/projects/" + project + "/analyses", json={"label": "online", "job": {"gear_id": gear, "inputs": {"csv": {"type": "acquisition", "id": acquisition, "name": "test.csv"}}}})
    assert r.ok
    ...
    r = as_drone.post("/engine", params={"level": "analysis", "id": analysis1_id, "job": job}, files=file_form("output.csv", meta={"type": "tabular data"}))
    assert r.ok

    # Create ad-hoc analysis for session
    r = as_admin.post("/sessions/" + session + "/analyses", json={"label": "analysis_label", "inputs": [{"type": "acquisition", "id": acquisition, "name": "test.csv"}]})
    assert r.ok
    analysis2_id = r.json()["_id"]

    # Manual upload
    r = as_admin.post("/analyses/" + analysis2_id + "/files", files=file_form("output1.csv", "output2.csv", meta=[{"name": "output1.csv", "info": {"foo": "foo"}}, {"name": "output2.csv", "info": {"bar": "bar"}}]))
    assert r.ok

    # Test no match
    # Retrieve a ticket for a batch download
    r = as_admin.post("/download", params={"type": "full", "analyses": "true", "metadata": "true"}, json={"filters": [{"tags": {"+": ["NONE-SUCH"]}}], "nodes": [{"level": "project", "_id": project}]})
    assert r.status_code == 404

    # Retrieve a ticket for a batch download
    r = as_admin.post("/download", params={"type": "full"}, json={"nodes": [{"level": "project", "_id": project}]})
    assert r.ok
    ticket = r.json()["ticket"]
    ...
    # Retrieve a ticket for a batch download
    r = as_admin.post("/download", params={"type": "full", "metadata": "true"}, json={"filters": [{"tags": {"-": ["minus"]}}], "nodes": [{"level": "project", "_id": project}]})
    assert r.ok
    ticket = r.json()["ticket"]
    ...
    # Retrieve a ticket for a batch download
    r = as_admin.post("/download", params={"type": "full", "metadata": "true"}, json={"filters": [{"tags": {"+": ["acq1"]}}], "nodes": [{"level": "project", "_id": project}]})
    assert r.ok
    ticket = r.json()["ticket"]
    ...
    r = as_admin.post("/download", params={"type": "full", "metadata": "true", "analyses": "true"}, json={"filters": [{"tags": {"-": ["minus"]}}], "nodes": [{"level": "project", "_id": project}]})
    assert r.ok
    ticket = r.json()["ticket"]
