...
def test_site_rules(randstr, data_builder, as_admin, as_user, as_public):
    ...
    rule = {"gear_id": gear, "name": "csv-job-trigger-rule", "any": [], "not": [], "all": [{"type": "file.type", "value": "tabular data"}]}
    ...
    r = as_admin.post("/site/rules", json={"gear_id": gear, "name": "no-conditions-rule", "any": [], "not": [], "all": []})
    assert r.status_code == 400
    assert "conditions" in r.json()["message"]

    # attempt to add site rule with invalid regex
    invalid_pattern = "^(?non-image$).+"
    r = as_admin.post("/site/rules", json={"gear_id": gear, "name": "invalid-regex-rule", "any": [], "not": [], "all": [{"type": "file.classification", "value": invalid_pattern, "regex": True}]})
    assert r.status_code == 422
    assert invalid_pattern in r.json()["patterns"]

    # try to add rule with id in the payload
    r = as_admin.post("/site/rules", json={"gear_id": gear, "name": "invalid-regex-rule", "_id": "foo", "any": [], "not": [], "all": [{"type": "file.classification", "value": "Functional"}]})
    assert r.status_code == 400
    ...
    r = as_admin.put("/site/rules/" + rule_id, json={"all": []})
    assert r.status_code == 400

    # attempt to modify site rule with invalid regex
    r = as_admin.put("/site/rules/" + rule_id, json={"all": [{"type": "file.classification", "value": invalid_pattern, "regex": True}]})
    assert r.status_code == 422
    assert invalid_pattern in r.json()["patterns"]
...
def test_site_rules_copied_to_new_projects(randstr, data_builder, file_form, as_admin, as_root):
    ...
    rule_1 = {"gear_id": gear_1, "name": "csv-job-trigger-rule", "any": [], "not": [], "all": [{"type": "file.type", "value": "tabular data"}]}

    gear_2 = data_builder.create_gear(gear={"version": "0.0.1"})
    rule_2 = {"gear_id": gear_2, "name": "text-job-trigger-rule", "any": [], "not": [], "all": [{"type": "file.type", "value": "text"}]}
    ...
    r = as_admin.post("/projects", json={"group": group, "label": "project_1"})
    assert r.ok
    ...
    r = as_admin.post(
        "/upload/label",
        files=file_form("acquisition.csv", meta={"group": {"_id": group}, "project": {"label": "test_project"}, "session": {"label": "test_session_label", "subject": {"code": "test_subject_code"}}, "acquisition": {"label": "test_acquisition_label", "files": [{"name": "acquisition.csv"}]}}),
    )
    assert r.ok
...
def test_project_rules(randstr, data_builder, file_form, as_root, as_admin, with_user, api_db):
    ...
    r = as_admin.post("/projects/" + project + "/permissions", json={"_id": with_user.user, "access": "ro"})
    assert r.ok
    ...
    rule_json = {"gear_id": "000000000000000000000000", "name": "csv-job-trigger-rule", "any": [], "not": [], "all": [{"type": "file.type", "value": "tabular data"}]}
    ...
    r = as_admin.post(
        ...
        json={"gear_id": gear, "name": "txt-job-trigger-rule-with-classification", "any": [{"type": "container.has-classification", "value": "functional"}, {"type": "container.has-classification", "value": "anatomical"}], "all": [{"type": "file.type", "value": "text"}], "not": []},
    )
    assert r.ok
    rule2 = r.json()["_id"]
    ...
    metadata = {"project": {"label": "rule project", "files": [{"name": "test2.csv", "classification": {"intent": ["functional"]}}]}}

    r = as_admin.post("/engine", params={"level": "project", "id": project}, files=file_form(meta=metadata))
    assert r.ok
    ...
    # add regex rule
    # NOTE this is a legacy rule
    r = as_admin.post("/projects/" + project + "/rules", json={"gear_id": gear, "name": "file-classification-regex", "any": [], "not": [], "all": [{"type": "file.name", "value": "test\d+\.(csv|txt)", "regex": True}]})
    assert r.ok
    rule3 = r.json()["_id"]
    ...
    # add modality rule
    r = as_admin.post("/projects/" + project + "/rules", json={"gear_id": gear, "name": "file-modality-rule", "any": [], "not": [], "all": [{"type": "file.modality", "value": "mr"}]})
    assert r.ok
    rule4 = r.json()["_id"]
    ...
    metadata = {"project": {"label": "rule project", "files": [{"name": "test_modality.txt", "modality": "MR"}]}}
...
def test_context_input_rule(randstr, data_builder, default_payload, api_db, as_admin, file_form):
    ...
    gear_doc["gear"]["inputs"] = {"A": {"base": "context"}, "text-file": {"base": "file", "type": {"enum": ["text"]}}}

    r = as_admin.post("/gears/" + gear_name, json=gear_doc)
    assert r.ok
    gear = r.json()["_id"]

    r = as_admin.post("/projects/" + project + "/rules", json={"gear_id": gear, "name": "context-input-trigger-rule", "any": [], "not": [], "all": [{"type": "file.type", "value": "text"}]})
    assert r.ok
    rule = r.json()["_id"]
    ...
    r = as_admin.post("/sessions/" + session + "/info", json={"set": {"context": {"A": "session_context_value"}}})
    assert r.ok
...
def test_disabled_rules(randstr, data_builder, api_db, as_admin, file_form):
    ...
    gear = data_builder.create_gear(gear={"version": "0.0.1"})
    project = data_builder.create_project()
    r = as_admin.post("/projects/" + project + "/rules", json={"gear_id": gear, "name": "csv-job-trigger-rule", "any": [], "not": [], "all": [{"type": "file.type", "value": "tabular data"}], "disabled": True})
    assert r.ok
    rule = r.json()["_id"]
...
def test_auto_update_rules(data_builder, api_db, as_admin):
    ...
    r = as_admin.post("/projects/" + project + "/rules", json={"gear_id": gearv1, "name": "test-auto-update-rule", "any": [], "not": [], "all": [{"type": "file.type", "value": "tabular data"}], "disabled": False, "config": {"param": True}, "auto_update": True})
    assert r.status_code == 400

    # Post with only auto-update
    r = as_admin.post("/projects/" + project + "/rules", json={"gear_id": gearv1, "name": "test_auto_update_rule", "any": [], "not": [], "all": [{"type": "file.type", "value": "tabular data"}], "disabled": False, "auto_update": True})
    assert r.ok
    rule_id = r.json()["_id"]
    ...
    r = as_admin.put("/projects/" + project + "/rules/" + rule_id, json={"config": {"param": True}})
    assert r.status_code == 400

    # Unset auto_update and set config
    r = as_admin.put("/projects/" + project + "/rules/" + rule_id, json={"auto_update": False, "config": {"param": True}})
    assert r.ok

    # Unset config
    r = as_admin.put("/projects/" + project + "/rules/" + rule_id, json={"config": {}})
    assert r.ok
    ...
    r = as_admin.put("/projects/" + project + "/rules/" + rule_id, json={"config": {"param": True}, "auto_update": True})
    assert r.status_code == 400
    ...
    r = as_admin.put("/projects/" + project + "/rules/" + rule_id, json={"auto_update": True})
    assert r.ok
    ...
    r = as_admin.put("/projects/" + project + "/rules/" + rule_id, json={"auto_update": False})
    assert r.ok
    ...
    r = as_admin.put("/projects/" + project + "/rules/" + rule_id, json={"gear_id": gearv2, "auto_update": True})
    assert r.ok
    ...
    r = as_admin.put("/projects/" + project + "/rules/" + rule_id, json={"gear_id": gearv1})
    assert r.status_code == 400
    ...
    r = as_admin.put("/projects/" + project + "/rules/" + rule_id, json={"gear_id": gearv2})
    assert r.ok
...
def test_rules_rerun_after_file_replace(randstr, data_builder, file_form, as_root, as_admin, with_user, api_db):
    ...
    def simulate_engine_run(job_id, acquisition_id, files):
        ...
        assert as_root.post("/engine", params={"level": "acquisition", "id": acquisition_id, "job": job_id}, files=files).ok
    ...
    classifier_rule = {"gear_id": classifier_gear, "name": "classifer-rule", "any": [], "not": [], "all": [{"type": "file.type", "value": "dicom"}]}

    # Run converter gear when classifier has complete
    converter_rule = {"gear_id": converter_gear, "name": "converter-rule", "any": [], "not": [{"type": "file.classification", "value": "Non-Image"}], "all": [{"type": "file.type", "value": "dicom"}, {"regex": True, "type": "file.classification", "value": ".+"}]}

    # Run QA gear when converter has complete
    qa_rule = {"gear_id": qa_gear, "name": "qa-rule", "any": [{"type": "file.classification", "value": "functional"}], "not": [], "all": [{"type": "file.type", "value": "nifti"}]}
    ...
    r = as_admin.post("/upload/reaper", files=file_form(dicom_file_name, meta={"group": {"_id": group}, "project": {"label": project_label}, "session": {"uid": session_uid}, "acquisition": {"uid": session_uid, "files": [{"name": dicom_file_name}]}}))
    assert r.ok
    ...
    # Simulate engine run
    payload = file_form(meta={"acquisition": {"files": [{"name": dicom_file_name, "classification": {"Intent": ["Functional"]}}]}})
    ...
    payload = file_form(nifti_file_name, meta={"acquisition": {"files": [{"name": nifti_file_name, "classification": {"Intent": ["Functional"]}}]}})
    simulate_engine_run(job_id, acquisition, payload)
    ...
    # Simulate engine runs
    payload = file_form(qa_file_name, meta={"acquisition": {"files": [{"name": qa_file_name}]}})
    simulate_engine_run(job_id, acquisition, payload)
    ...
    r = as_admin.post("/upload/reaper", files=file_form((dicom_file_name, "new_file_content"), meta={"group": {"_id": group}, "project": {"label": project_label}, "session": {"uid": session_uid}, "acquisition": {"uid": session_uid, "files": [{"name": dicom_file_name}]}}))
    assert r.ok
    ...
    # Simulate engine run
    payload = file_form((nifti_file_name, "new_nifti_content"), meta={"acquisition": {"files": [{"name": nifti_file_name, "classification": {"Intent": ["Functional"]}}]}})
    simulate_engine_run(job_id, acquisition, payload)
...
def test_optional_input_gear_rules(default_payload, data_builder, api_db, as_admin, as_root, file_form):
    ...
    gear_doc["gear"]["inputs"] = {"text": {"base": "file", "name": {"pattern": "^.*.txt$"}, "size": {"maximum": 100000}}, "csv": {"base": "file", "name": {"pattern": "^.*.csv$"}, "size": {"maximum": 100000}, "optional": True}, "api_key": {"base": "api-key", "read-only": True}}
    ...
    # Try posting rule with config and that auto-updates
    r = as_admin.post("/projects/" + project + "/rules", json={"gear_id": gear, "name": "test-optional-input-rule", "any": [], "not": [], "all": [{"type": "file.type", "value": "text"}], "disabled": False, "auto_update": True})
    assert r.ok
    rule_id = r.json()["_id"]
...
def test_multi_input_rules(default_payload, data_builder, as_admin, as_root, file_form):
    ...
    gear_doc = default_payload["gear"]
    # Try creating batch with optional inputs and api-key input
    gear_doc["gear"]["inputs"] = {"text": {"base": "file", "name": {"pattern": "^.*.txt$"}, "size": {"maximum": 100000}}, "csv": {"base": "file", "name": {"pattern": "^.*.csv$"}, "size": {"maximum": 100000}}}
    ...
    r = as_admin.post(
        "/projects/" + project + "/rules",
        json={"gear_id": gear, "name": "test-fixed-input-rule", "any": [], "not": [], "all": [{"type": "file.type", "value": "text"}], "disabled": False, "auto_update": True, "fixed-inputs": [{"input": "csv", "name": "test.csv", "id": acquisition, "type": "acquisition"}]},
    )
    assert r.status_code == 400
    ...
    r = as_admin.post(
        ...
        json={
            ...
            "disabled": False,
            "auto_update": False,
            "fixed-inputs": [{"input": "csv", "name": "test.csv", "id": acquisition, "type": "acquisition"}, {"input": "text", "name": "test.csv", "id": acquisition, "type": "acquisition"}],
        },
    )
    assert r.status_code == 400
    ...
    r = as_admin.post(
        "/projects/" + project + "/rules",
        json={"gear_id": gear, "name": "test-fixed-input-rule", "any": [], "not": [], "all": [{"type": "file.type", "value": "text"}], "disabled": False, "auto_update": True, "fixed-inputs": [{"input": "not-an-input", "name": "test.csv", "id": acquisition, "type": "acquisition"}]},
    )
    assert r.status_code == 400
    ...
    r = as_admin.post(
        "/projects/" + project + "/rules",
        json={"gear_id": gear, "name": "test-fixed-input-rule", "any": [], "not": [], "all": [{"type": "file.type", "value": "text"}], "disabled": False, "auto_update": False, "fixed_inputs": [{"input": "csv", "name": "test-doesnt-exist.csv", "id": acquisition, "type": "acquisition"}]},
    )
    assert r.status_code == 404

    # try creating a site rule with fixed inputs
    r = as_admin.post(
        "/site/rules", json={"gear_id": gear, "name": "test-fixed-input-rule", "any": [], "not": [], "all": [{"type": "file.type", "value": "text"}], "disabled": False, "auto_update": False, "fixed_inputs": [{"input": "csv", "name": "test.csv", "id": acquisition, "type": "acquisition"}]}
    )
    assert r.status_code == 400
    ...
    r = as_admin.post(
        "/projects/" + project + "/rules",
        json={"gear_id": gear, "name": "test-fixed-input-rule", "any": [], "not": [], "all": [{"type": "file.type", "value": "text"}], "disabled": False, "auto_update": False, "fixed_inputs": [{"input": "csv", "name": "test.csv", "id": acquisition, "type": "acquisition"}]},
    )
    assert r.ok
...
def test_project_rule_providers(site_providers, data_builder, file_form, as_root, as_admin, as_user, as_drone, api_db):
    ...
    assert as_admin.post("/projects/" + project + "/permissions", json={"_id": uid, "access": "admin"}).ok

    # User cannot set compute_provider_id
    rule_orig = {"all": [{"type": "file.type", "value": "tabular data"}], "any": [], "gear_id": gear, "name": "csv-job-trigger-rule", "not": []}
...
def test_analysis_gear_rules(data_builder, as_admin, file_form, api_db):
    ...
    r = as_admin.post("/sessions/" + session + "/analyses", json={"label": "Test-Rule-Analysis"})
    assert r.ok
    analysis = r.json()["_id"]
