...
def upload_file_form(file_form, merge_dict, randstr):
    def create_form(**meta_override):
        ...
        meta = {
            "project": {"label": prefix + "-project-label", "files": [{"name": files["project"]}], "tags": ["one", "two"]},
            "session": {"uid": prefix + "-session-uid", "label": prefix + "-session-label", "subject": {"code": prefix + "-subject-code"}, "files": [{"name": files["session"]}], "tags": ["one", "two"]},
            "acquisition": {"uid": prefix + "-acquisition-uid", "label": prefix + "-acquisition-label", "files": [{"name": files["acquisition"]}], "tags": ["one", "two"]},
        }
        if meta_override:
...
def test_reaper_upload(data_builder, randstr, upload_file_form, as_admin, as_root, as_user):
    ...
    r = as_admin.post("/upload/reaper", files=upload_file_form(group={"_id": group_1}, project={"label": project_label_1}, session={"uid": session_uid}))
    assert r.ok

    # reaper-upload files to group_1/project_label_1 using session_uid without any files
    file_form = upload_file_form(group={"_id": group_1}, project={"label": project_label_1, "files": []}, session={"uid": session_uid + "1", "files": [], "subject": {"code": prefix + "-subject-code", "files": []}})
    ...
    r = as_root.post("/upload/reaper", files=upload_file_form(group={"_id": ""}, project={"label": ""}, session={"uid": session_uid + "1"}))
    assert r.ok
    ...
    # No group given
    r = as_root.post("/upload/reaper", files=upload_file_form(group={"_id": ""}, project={"label": project_label_1}, session={"uid": session_uid + "2"}))
    assert r.ok
    ...
    r = as_root.post("/upload/reaper", files=upload_file_form(group={"_id": group_3}, project={"label": ""}, session={"uid": session_uid + "3"}))
    assert r.ok
    # get session created by the upload
    ...
    r = as_root.post("/upload/reaper", files=upload_file_form(group={"_id": group_3}, project={"label": "Miss-typed project"}, session={"uid": session_uid + "4"}))
    assert r.ok
    # get session created by the upload
    r = as_root.get("/groups/" + group_3 + "/projects")
    assert r.ok
    ...
    r = as_user.post("/upload/reaper", files=upload_file_form(group={"_id": group_3}, project={"label": "Miss-typed project"}, session={"uid": session_uid + "4"}))
    assert r.status_code == 403

    # Test reaper uploads for embedded subject metadata
    r = as_admin.post("/upload/reaper", files=upload_file_form(group={"_id": group_1}, project={"label": project_label_1}, session={"uid": session_uid + "5", "subject": {"code": "embedded", "lastname": "Lastname"}}))
    assert r.ok
    ...
    expected_raw_subject = {"lastname": "Lastname"}
...
def test_label_upload_unknown_group_project(data_builder, file_form, as_root, as_admin):
    ...
    r = as_admin.post("/upload/label", files=file_form("acquisition.csv", meta={"group": {"_id": ""}, "project": {"label": ""}, "session": {"label": "test_session_label"}, "acquisition": {"label": "test_acquisition_label", "files": [{"name": "acquisition.csv"}]}}))
    assert r.ok
    ...
    r = as_admin.post("/upload/label", files=file_form("acquisition.csv", meta={"group": {"_id": ""}, "project": {"label": ""}, "session": {"label": "test_session_label_2"}, "acquisition": {"label": "test_acquisition_label_2", "files": [{"name": "acquisition.csv"}]}}))
    assert r.ok
    ...
    r = as_admin.post("/upload/label", files=file_form("acquisition.csv", meta={"group": {"_id": "not_a_real_group"}, "project": {"label": "new_project"}, "session": {"label": "test_session_label"}, "acquisition": {"label": "test_acquisition_label", "files": [{"name": "acquisition.csv"}]}}))
    assert r.ok

    # Try uploading 0 files
    r = as_admin.post(
        "/upload/label",
        files={"metadata": file_form("acquisition.csv", meta={"group": {"_id": "not_a_real_group"}, "project": {"label": "new_project"}, "session": {"label": "test_session_label"}, "acquisition": {"label": "test_acquisition_label", "files": [{"name": "acquisition.csv"}]}}).get("metadata")},
    )
    assert r.status_code == 400
    ...
    r = as_admin.post("/upload/label", files=file_form("acquisition.csv", meta={"group": {"_id": group1}, "project": {"label": ""}, "session": {"label": "test_session_label"}, "acquisition": {"label": "test_acquisition_label", "files": [{"name": "acquisition.csv"}]}}))
    assert r.ok
...
def test_label_project_search(data_builder, file_form, as_admin, as_root):
    ...
    # Upload with group 1
    r = as_admin.post("/upload/label", files=file_form("acquisition.csv", meta={"group": {"_id": group_label_1}, "project": {"label": ""}, "session": {"label": "test_session_label"}, "acquisition": {"label": "test_acquisition_label", "files": [{"name": "acquisition.csv"}]}}))
    assert r.ok
    ...
    r = as_admin.post("/upload/label", files=file_form("acquisition.csv", meta={"group": {"_id": group_label_2}, "project": {"label": ""}, "session": {"label": "test_session_label"}, "acquisition": {"label": "test_acquisition_label", "files": [{"name": "acquisition.csv"}]}}))
    assert r.ok
    ...
    r = as_admin.post("/upload/label", files=file_form("acquisition.csv", meta={"group": {"_id": group_label_3}, "project": {"label": ""}, "session": {"label": "test_session_label_2"}, "acquisition": {"label": "test_acquisition_label_2", "files": [{"name": "acquisition.csv"}]}}))
    assert r.ok
...
def test_reaper_reupload_deleted(data_builder, as_admin, as_root, file_form):
    ...
    reap_data = file_form("reaped.txt", meta={"group": {"_id": "reupload"}, "project": {"label": "reupload"}, "session": {"uid": "reupload"}, "acquisition": {"uid": "reupload", "files": [{"name": "reaped.txt"}]}})
...
def test_uid_upload(data_builder, file_form, as_admin, as_user, as_public):
    ...
    uid_meta = {"group": {"_id": group}, "project": {"label": "uid_upload", "files": [{"name": "project.csv"}]}, "session": {"uid": "uid_upload", "subject": {"code": "uid_upload"}, "files": [{"name": "session.csv"}]}, "acquisition": {"uid": "uid_upload", "files": [{"name": "acquisition.csv"}]}}
...
def test_label_upload(data_builder, file_form, as_admin):
    ...
    r = as_admin.post(
        ...
        files=file_form(
            ...
            meta={
                "group": {"_id": group},
                "project": {"label": "test_project", "files": [{"name": "project.csv"}]},
                "session": {"label": "test_session_label", "subject": {"code": "test_subject_code", "firstname": "Name1", "files": [{"name": "subject.csv"}]}, "files": [{"name": "session.csv"}]},
                "acquisition": {"label": "test_acquisition_label", "files": [{"name": "acquisition.csv"}]},
            },
        ),
    )
    assert r.ok
    ...
    r = as_admin.post(
        ...
        files=file_form(
            ...
            meta={
                "group": {"_id": group},
                "project": {"label": "test_project", "files": [{"name": "project.csv"}]},
                "session": {"label": "test_session2_label", "subject": {"code": "test_subject_code", "firstname": "Name2"}, "files": [{"name": "session.csv"}]},  # Note that we don't upload a file here
                "acquisition": {"label": "test_acquisition_label", "files": [{"name": "acquisition.csv"}]},
            },
        ),
    )
    ...
    r = as_admin.post(
        ...
        files=file_form(
            ...
            meta={
                "group": {"_id": group},
                "project": {"label": "test_project", "files": [{"name": "project.csv"}]},
                "session": {"label": "test_session2_label", "subject": {"code": "test_subject_code", "firstname": "Name3"}, "files": [{"name": "session.csv"}]},  # Note that we don't upload a file here
                "acquisition": {"label": "test_acquisition_label2", "files": [{"name": "acquisition.csv"}]},
            },
        ),
    )
    assert r.ok
...
def test_master_subject_code_upload(data_builder, file_form, as_admin):
    ...
    metadata = {"group": {"_id": group}, "project": {"label": "test_project"}, "session": {"label": "test_session_label", "subject": {"code": "test_code", "master_code": "non_existent"}}, "acquisition": {"label": "test_acquisition_label", "files": [{"name": "acquisition.csv"}]}}
    # invalid master subject code
    r = as_admin.post("/upload/label", files=file_form("acquisition.csv", meta=metadata))
    assert r.status_code == 400

    # create a valid master subject code
    r = as_admin.post("/subjects/master-code", json={"use_patient_id": True, "patient_id": "MRN-123456"})
    assert r.ok
    ...
    # it should fail
    # create a valid master subject code
    r = as_admin.post("/subjects/master-code", json={"use_patient_id": True, "patient_id": "MRN-56789"})
    assert r.ok
...
def test_multi_upload(data_builder, upload_file_form, randstr, as_admin):
    ...
    fixed_uid_form_args = dict(group={"_id": fixed_uid_group}, project={"label": fixed_uid + "-project-label"}, session={"uid": fixed_uid + "-fixed-uid"}, acquisition={"uid": fixed_uid + "-fixed-uid"})
    ...
    fixed_label_form_args = dict(group={"_id": fixed_label_group}, project={"label": fixed_label + "-project-label"}, session={"label": fixed_label + "-fixed-label", "subject": {"code": fixed_label + "-subject-code"}}, acquisition={"label": fixed_label + "-fixed-label"})
...
def test_acquisition_engine_upload(data_builder, file_form, as_admin, api_db):
    ...
    job = data_builder.create_job(inputs={"test": {"type": "acquisition", "id": acquisition, "name": "test.txt"}})

    metadata = {
        "project": {"label": "engine project", "info": {"test": "p"}, "tags": ["one", "two"]},
        "session": {"label": "engine session", "subject": {"code": "engine subject", "sex": "male", "age": 100000000000}, "info": {"test": "s", "file.txt": "Hello"}, "tags": ["one", "two"]},
        "acquisition": {"label": "engine acquisition", "timestamp": "2016-06-20T21:57:36+00:00", "info": {"test": "a"}, "files": [{"name": "one.csv", "type": "engine type 0", "info": {"test": "f0"}}, {"name": "two.csv", "type": "engine type 1", "info": {"test": "f1"}}], "tags": ["one", "two"]},
    }

    # try engine upload w/ non-existent job_id
    r = as_admin.post("/engine", params={"level": "acquisition", "id": acquisition, "job": "000000000000000000000000"}, files=file_form("one.csv", "two.csv", meta=metadata))
    assert r.status_code == 404

    metadata["acquisition"]["files"] = [{"name": "one.csv", "type": "engine type 0", "info": {"test": "f0"}}, {"name": "folderA/two.csv", "type": "engine type 1", "info": {"test": "f1"}}, {"name": "../folderB/two.csv", "type": "engine type 1", "info": {"test": "f1"}}]

    # engine upload with slashes in filenames with filename_path=true
    r = as_admin.post("/engine", params={"level": "acquisition", "id": acquisition, "job": job, "filename_path": True}, files=file_form("one.csv", "folderA/two.csv", "../folderB/two.csv", meta=metadata))
    assert r.ok
    ...
    metadata["acquisition"]["files"] = [{"name": "one.csv", "type": "engine type 0", "info": {"test": "f0"}}, {"name": "folderA/two.csv", "type": "engine type 1", "info": {"test": "f1"}}]

    r = as_admin.post("/engine", params={"level": "acquisition", "id": acquisition, "job": job, "filename_path": False}, files=file_form("one.csv", "folderA/two.csv", meta=metadata))
    assert r.ok
    r = as_admin.get("/acquisitions/" + acquisition)
    assert r.ok
    a = r.json()
...
def test_session_engine_upload(data_builder, file_form, as_admin):
    ...
    metadata = {
        ...
        "session": {
            ...
            "subject": {"code": "engine subject", "race": "Asian"},
            "timestamp": "2016-06-20T21:57:36+00:00",
            "info": {"test": "s"},
            "files": [{"name": "one.csv", "type": "engine type 0", "info": {"test": "f0"}}, {"name": "two.csv", "type": "engine type 1", "info": {"test": "f1"}}, {"name": "folder/three.csv", "type": "engine type 2", "info": {"test": "f2"}}],
            "tags": ["one", "two"],
        },
    }

    r = as_admin.post("/engine", params={"level": "session", "id": session, "filename_path": True}, files=file_form("one.csv", "two.csv", "folder/three.csv", meta=metadata))
    assert r.ok
...
def test_project_engine_upload(data_builder, file_form, as_admin):
    ...
    r = as_admin.post("/engine", params={"level": "project", "id": project, "filename_path": True}, files=file_form("one.csv", "two.csv", "folder/three.csv", meta=metadata))
    assert r.ok
...
def test_acquisition_file_only_engine_upload(data_builder, file_form, as_admin):
    ...
    r = as_admin.post("/engine", params={"level": "acquisition", "id": acquisition}, files=file_form(*file_names))
    assert r.ok
...
def test_acquisition_subsequent_file_engine_upload(data_builder, file_form, as_admin):
    ...
    file_name_1 = "one.csv"
    r = as_admin.post("/engine", params={"level": "acquisition", "id": acquisition}, files=file_form(file_name_1, meta={"acquisition": {"files": [{"name": file_name_1, "type": "engine type 1", "info": {"test": "f1"}}]}}))
    assert r.ok
    ...
    file_name_2 = "two.csv"
    r = as_admin.post("/engine", params={"level": "acquisition", "id": acquisition}, files=file_form(file_name_2, meta={"acquisition": {"files": [{"name": file_name_2, "type": "engine type 2", "info": {"test": "f2"}}]}}))
    assert r.ok
...
def test_acquisition_metadata_only_engine_upload(data_builder, file_form, as_admin):
    ...
    metadata = {
        "project": {"label": "engine project", "info": {"test": "p"}, "tags": ["one", "two"]},
        "session": {"label": "engine session", "subject": {"code": "engine subject", "race": "Asian", "age": 100200300}, "info": {"test": "s"}, "tags": ["one", "two"]},
        "acquisition": {"label": "engine acquisition", "timestamp": "2016-06-20T21:57:36+00:00", "info": {"test": "a"}, "tags": ["one", "two"]},
    }

    r = as_admin.post("/engine", params={"level": "acquisition", "id": acquisition}, files=file_form(meta=metadata))
    assert r.ok
...
def test_analysis_upload(data_builder, default_payload, file_form, as_admin):
    ...
    r = as_admin.post("/sessions/" + session + "/analyses", files=file_form("one.csv", meta={"label": "test analysis", "inputs": [{"name": "one.csv"}]}))
    assert r.ok
    ...
    r = as_admin.post("/acquisitions/" + acquisition + "/analyses", files=file_form("one.csv", meta={"label": "test analysis", "inputs": [{"name": "one.csv"}]}))
    assert r.ok
    ...
    r = as_admin.post("/sessions/" + session + "/analyses", json={"label": "test analysis job", "job": {"gear_id": gear, "inputs": {"csv": {"type": "acquisition", "id": acquisition, "name": "one.csv"}}, "tags": ["example"]}})
    assert r.ok
    session_analysis = r.json()["_id"]
...
def test_analysis_engine_upload(data_builder, file_form, as_admin):
    ...
    r = as_admin.post("/engine", params={"level": "analysis", "id": session_analysis}, files=file_form("out.csv", meta={"type": "text", "value": {"label": "test"}, "enabled": True}))
    assert r.ok
...
def test_packfile_upload(data_builder, file_form, as_user, as_admin, api_db):
    ...
    # upload to packfile
    r = as_admin.post("/projects/" + project + "/packfile", params={"token": token}, files=file_form("one.csv"))
    assert r.ok

    # upload another one to packfile
    r = as_admin.post("/projects/" + project + "/packfile", params={"token": token}, files=file_form("two.csv"))
    assert r.ok

    # Upload another one to packfile
    r = as_admin.post("/projects/" + project + "/packfile", params={"token": token}, files=file_form("three.csv"))
    assert r.ok

    metadata_json = json.dumps(
        {"project": {"_id": project}, "session": {"label": "test-packfile-label (session)", "subject": {"code": "subj-01", "ethnicity": "Hispanic or Latino"}}, "acquisition": {"label": "test-packfile-label (acquisition)", "timestamp": "1979-01-01T00:00:00+00:00"}, "packfile": {"type": "test"}}
    )

    # try to finish packfile-upload w/o token
    r = as_admin.post("/projects/" + project + "/packfile-end", params={"metadata": metadata_json})
    assert r.status_code == 500

    # try to finish packfile-upload with files in the request
    r = as_admin.post("/projects/" + project + "/packfile-end", params={"token": token, "metadata": metadata_json}, files={"file": ("packfile-end.txt", "sending files to packfile-end is not allowed\n")})
    assert r.status_code == 500

    # finish packfile-upload (creates new session/acquisition)
    r = as_admin.post("/projects/" + project + "/packfile-end", params={"token": token, "metadata": metadata_json})
    assert r.ok
    ...
    metadata_json = json.dumps({"project": {"_id": project}, "session": {"label": "test-packfile-timestamp"}, "acquisition": {"label": "test-packfile-timestamp", "timestamp": "1990-01-01T00:00:00+00:00"}, "packfile": {"type": "test"}})
    ...
    metadata_json = json.dumps({"project": {"_id": project}, "session": {"label": "test-packfile-timestamp", "subject": {"code": "new-subject"}}, "acquisition": {"label": "test-packfile-timestamp", "timestamp": "1999-01-01T00:00:00+00:00"}, "packfile": {"type": "test"}})
    ...
    files = [("file", file_form("two.csv")["file"]), ("file", file_form("three.csv")["file"])]

    # upload to packfile
    r = as_user.post("/projects/" + project + "/packfile", params={"token": token}, files=files)
    assert r.ok
...
def test_engine_tags(data_builder, file_form, as_root):
    ...
    metadata = {
        "project": {
            "label": "engine project",
            "info": {"test": "p"},
            "files": [{"name": "one.csv", "type": "engine type 0", "info": {"test": "f0"}, "tags": ["ein", "zwei"]}, {"name": "two.csv", "type": "engine type 1", "info": {"test": "f1"}}, {"name": "folder/three.csv", "type": "engine type 2", "info": {"test": "f2"}}],
            "tags": ["one", "two"],
        }
    }

    r = as_root.post("/engine", params={"level": "project", "id": project, "filename_path": True}, files=file_form("one.csv", "two.csv", "folder/three.csv", meta=metadata))
    assert r.ok
    ...
    metadata_two = {"project": {"label": "override test", "tags": ["two", "three"]}}
    r = as_root.post("/engine", params={"level": "project", "id": project}, files=file_form(meta=metadata_two))
    assert r.ok
