...
VALID_PROVIDER = {"provider_class": "compute", "provider_type": "static", "label": "My Provider", "config": {}}
...
def test_create_providers(api_db, as_admin, as_user, as_public):
    ...
    r = as_admin.post("/site/providers", json={"provider_class": "compute", "provider_type": "other", "label": "Label", "config": {}})
    assert r.status_code == 422

    # Unknown provider type
    r = as_admin.post("/site/providers", json={"provider_class": "other", "provider_type": "static", "label": "Label", "config": {}})
    assert r.status_code == 400  # Validated by enum type in schema

    # Validate static configuration
    r = as_admin.post("/site/providers", json={"provider_class": "compute", "provider_type": "static", "label": "Label", "config": {"value": "something"}})
    assert r.status_code == 422

    # No additional properties
    r = as_admin.post("/site/providers", json={"provider_class": "compute", "provider_type": "static", "label": "Label", "config": {}, "extra": True})
    assert r.status_code == 400
