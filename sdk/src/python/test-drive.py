...
userId = fw.add_user({"_id": email, "email": email, "firstname": rand_string(), "lastname": rand_string()})
...
## Groups
#
groupId = fw.add_group({"_id": rand_string()})
...
## Projects
#
projectId = fw.add_project({"label": rand_string(), "group": groupId})
...
## Sessions
#
sessionId = fw.add_session({"label": rand_string(), "project": projectId})
...
analysisId = fw.add_session_analysis(sessionId, {"label": "testdrive", "inputs": [{"type": "session", "id": sessionId, "name": filename}]})
...
## Acquisitions
#
acqId = fw.add_acquisition({"label": rand_string(), "session": sessionId})
...
## Collections
#
colId = fw.add_collection({"label": rand_string(), "description": rand_string()})
...
gearId = fw.add_gear(
    ...
    {
        "category": "converter",
        "exchange": {"git-commit": "example", "rootfs-hash": "sha384:example", "rootfs-url": "https://example.example"},
        "gear": {
...
job_id = fw.add_job({"gear_id": gearId, "state": "pending", "inputs": {"x": {"type": "acquisition", "id": acqId, "name": "test-drive.py"}}})
