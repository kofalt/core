...
class SubjectsTestCases(SdkTestCase):
    ...
    def test_subjects(self):
        ...
        subject = flywheel.Subject(project=self.project_id, firstname=self.rand_string(), lastname=self.rand_string(), code=subject_code, sex="other", info={"some-subject-key": 37})
    ...
    def test_subject_files(self):
        ...
        resp = fw.replace_subject_file_classification(subject_id, "yeats.txt", {"Custom": ["measurement1", "measurement2"]})
        self.assertEqual(resp.modified, 1)
        self.assertEqual(resp.jobs_spawned, 0)

        r_subject = fw.get_subject(subject_id)
        self.assertEqual(r_subject.files[0].classification, {"Custom": ["measurement1", "measurement2"]})

        resp = fw.modify_subject_file_classification(subject_id, "yeats.txt", {"add": {"Custom": ["HelloWorld"]}, "delete": {"Custom": ["measurement2"]}})
        self.assertEqual(resp.modified, 1)
        self.assertEqual(resp.jobs_spawned, 0)

        r_subject = fw.get_subject(subject_id)
        self.assertEqual(r_subject.files[0].classification, {"Custom": ["measurement1", "HelloWorld"]})

        # Test file info
        self.assertEmpty(r_subject.files[0].info)
        fw.replace_subject_file_info(subject_id, "yeats.txt", {"a": 1, "b": 2, "c": 3, "d": 4})

        fw.set_subject_file_info(subject_id, "yeats.txt", {"c": 5})
    ...
    def test_subject_analysis(self):
        ...
        file_ref = flywheel.FileReference(id=subject_id, type="subject", name="yeats.txt")
...
def create_test_subject():
    ...
    return group_id, project_id, SdkTestCase.fw.add_subject({"project": project_id, "code": SdkTestCase.rand_string_lower(), "firstname": SdkTestCase.rand_string(), "lastname": SdkTestCase.rand_string(), "sex": "other", "age": 57})
